...
* The identity that runs a Fabric item using workspace identity authentication, whether a user or a service principal, must have an admin, member, or contributor role in the workspace so that Fabric can generate the workspace identity token.
