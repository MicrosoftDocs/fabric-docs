...
> [!NOTE]
> When you delete a workspace, Fabric also deletes its workspace identity, but not immediately. The workspace stays in a *Deleted* state for the duration of the [workspace retention period](../admin/workspace-retention.md), and Fabric doesn't delete the workspace identity until the workspace is permanently deleted. For details, see [Deleting the identity](#deleting-the-identity).
>
