...
3. In the Azure portal delete all corresponding private DNS zones
4. In the Azure portal delete the private link service **Microsoft.PowerBI/privateLinkServicesForPowerBI**. There should be only one. Turn on **Show hidden types** when exploring Resource Groups.
5. In the Fabric portal disable tenant-level private link: **Power BI => Admin portal => Tenant settings => Tenant-level Private Link**
