...
-  Run the recovered notebooks to recreate the MLVs in the new Lakehouse. For information about creating MLVs, see [Create a Materialized Lake View](../data-engineering/materialized-lake-views/create-materialized-lake-view#create-a-materialized-lake-view). If MLVs were also copied in the earlier step, run **CREATE OR REPLACE** while recreating them.
