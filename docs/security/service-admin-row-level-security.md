...
Row-level security (RLS) restricts data access for specific users of a Power BI semantic model. Filters restrict data at the row level, and you define filters within roles. In the Power BI service, users with access to a workspace have access to semantic models in that workspace. RLS only restricts data access for users with **Viewer** permissions. It doesn't apply to workspace **Admin**, **Member**, or **Contributor** roles.
...
You can configure RLS for imported semantic models in Power BI Desktop or the Power BI service. You can also configure RLS on semantic models that use DirectQuery, such as SQL Server. For Analysis Services or Azure Analysis Services live connections, you configure row-level security in the model, not in Power BI. The security option doesn't show up for live connection semantic models.

> [!NOTE]
> This article covers RLS for Power BI semantic models specifically. For data security in other Microsoft Fabric items, see [Security in Microsoft Fabric](/fabric/security/security-overview).
...
## Common DAX filter patterns for RLS roles

The following examples show common DAX filter expressions you can use when defining RLS roles in Power BI Desktop:
...
## Example: Filter Power BI sales data by region
...
> [!TIP]
> Use the **View as role** feature (described in [Validate the role within the Power BI service](#validate-the-role-within-the-power-bi-service)) to validate that your filter returns the expected rows before publishing the report.

## Bi-directional cross-filtering with RLS
...
## Manage security on your semantic model

To manage security on your semantic model, open the workspace where you saved your semantic model in Microsoft Fabric and do the following steps:

1. In Microsoft Fabric, select the **More options** menu for a semantic model. This menu appears when you hover on a semantic model name.
...
**Security** takes you to the Row-Level Security page where you add members to a role you created. Users with the workspace **Contributor** role or higher see the **Security** option and can assign users to a role. Semantic model ownership or **Build** permission might also be required depending on the scenario.

> [!NOTE]
> You can only manage security on semantic models that have row-level security roles already defined in Power BI Desktop or when editing your data model in the Power BI service. If your semantic model doesn't have roles already defined, you can't manage security in the Power BI service.

## Manage RLS role membership in the Power BI service

### Add members to an RLS role

In the Power BI service, you can add a member to an RLS role by typing in the email address or name of the user or security group. You can't add groups created in Power BI. You can add members [external to your organization](/power-bi/guidance/whitepaper-azure-b2b-power-bi#data-security-for-external-partners). For guidance on how RLS works with external B2B guest users, see [Considerations for external (B2B guest) users](#considerations-for-external-b2b-guest-users).

You can use the following Microsoft Entra ID and email-enabled groups to set up row-level security:
...
> [!IMPORTANT]
> Microsoft 365 groups aren't supported and can't be added to any RLS roles. Only the group types listed above are supported for RLS role membership.
...
### Remove members from an RLS role
...
## Validate the role within the Power BI service

You can validate that the RLS role you defined works correctly in the Power BI service by testing the role.
...
> [!NOTE]
> The **Test as role** feature doesn't work for DirectQuery semantic models with single sign-on (SSO) enabled. Additionally, not all aspects of a report can be validated in the **Test as role** feature, including Q&A visualizations, Quick insights visualizations, and Copilot.
...
> - For DirectQuery semantic models with SSO enabled, **Test as role** isn't supported. Instead, sign in as an actual Viewer-role user to validate data filtering.

[!INCLUDE [include-short-name](../includes/row-level-security-username.md)]

## Use RLS with workspaces in Power BI

If you publish your Power BI Desktop report to a [workspace](/power-bi/collaborate-share/service-new-workspaces) in the Power BI service, the RLS roles are applied to members assigned to the **Viewer** role in the workspace. Even if **Viewers** are given **Build** permissions to the semantic model, RLS still applies. For example, if Viewers with **Build** permissions use [Analyze in Excel](/power-bi/collaborate-share/service-analyze-in-excel), their view of the data is restricted by RLS. Workspace members assigned **Admin**, **Member**, or **Contributor** have edit permission for the semantic model and, therefore, RLS doesn't apply to them. If you want RLS to apply to people in a workspace, you can only assign them the **Viewer** role. For more information, see [roles in workspaces](/power-bi/collaborate-share/service-roles-new-workspaces).
...
### Microsoft Entra security groups with external members
...
**Recommended workaround:** Instead of adding external users to RLS roles through Microsoft Entra security groups, add them directly to the role by email address. The email address is resolved to the user's B2B account. This ensures their identity is correctly matched when RLS filters are applied. For more information, see [Manage RLS role membership in the Power BI service](#manage-rls-role-membership-in-the-power-bi-service).
...
If the issue persists after applying the workaround, see [Troubleshoot: External B2B guest sees no data in a Power BI report](#troubleshoot-external-b2b-guest-sees-no-data-in-a-power-bi-report) for additional diagnostic steps.

### UPN resolution for B2B guests in Power BI RLS
...
> [!IMPORTANT]
> If you use dynamic RLS with `USERPRINCIPALNAME()`, always test with actual external guest users. The [Test as role](#validate-the-role-within-the-power-bi-service) feature uses your own identity and won't reveal external user UPN resolution issues.
...
### Troubleshoot: External B2B guest sees no data in a Power BI report
