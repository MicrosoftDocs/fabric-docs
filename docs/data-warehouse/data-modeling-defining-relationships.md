...
In the model view, users can model their warehouse and the canonical autogenerated “default” dataset. We recommend modeling your data warehouse using Kimball methodologies, leveraging a star schema, wherever possible. There are two types of modeling possible:
...
The default dataset inherits all relationships between entities defined in the model view and infers them as Power BI dataset relationships, when objects are enabled for bi (Power BI Reports). By inheriting the warehouse’s business logic, a warehouse developer or BI analyst has decreased the time to value towards building a useful semantic model and metrics layer for analytical business intelligence (BI) reports in Power BI, Excel, or external tools like Tableau that read the XMLA format.
...
In Power BI, a dataset is always required before any reports can be built, so the default dataset enables quick reporting capabilities on top of the warehouse. Within the warehouse, a user can add warehouse objects - tables or views to their default dataset. They can also add additional semantic modeling properties, such as hierarchies and descriptions. These are then used to create the Power BI dataset’s tables. Users can also remove objects from the default dataset.
