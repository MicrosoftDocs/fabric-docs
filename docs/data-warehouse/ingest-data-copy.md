...
> [!NOTE]
> [!INCLUDE [fabric-dw](includes/fabric-dw.md)] also enables you to use [BULK INSERT](https://learn.microsoft.com/sql/t-sql/statements/bulk-insert-transact-sql?view=fabric) statement for data ingestion. The `COPY INTO` statement is the recommended statement for the new ingestion code, while the `BULK INSERT` statement enables you to reuse the code that you are using in SQL Server or Azure SQL.
