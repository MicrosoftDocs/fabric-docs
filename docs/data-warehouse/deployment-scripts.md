...
> [!TIP]
> When a script is **idempotent**, you can run it multiple times without causing problems. You can deploy to multiple databases without needing to predetermine their status.
...
A warehouse SQL project references the pre-deployment and post-deployment scripts in the project file (`.sqlproj`) by using the `PreDeploy` and `PostDeploy` item types. Each entry points to a single shared query file in the project.
...
1. Right-click the shared query you want to use.
    - If necessary, move a query from **My queries** to a shared query.
1. Select **Set deployment scripts**. This action takes you to the item settings pane.
1. In the **Pre-deployment script** and **Post-deployment script** fields, select **Choose script**.
1. Choose the **Set a query as a pre-/post-deployment script** option then select a shared query for each. Each dropdown lists all shared queries in the warehouse. 

### From item settings

1. Open the warehouse's **Settings**.
...
1. Add your Transact-SQL to the script and save the file. Ensure the file is saved in the `.sharedqueries` folder for the next Git update.
...
1. Add your Transact-SQL to the script and save the file. Ensure you save the file in the `.sharedqueries` folder for the next Git update.
1. Commit and push the changes to your Git repo, then update the workspace from source control to apply the configuration.
...
If you migrate a `.sqlproj` from SQL Server Data Tools (SSDT) or Visual Studio that already contains `<PreDeploy>` and `<PostDeploy>` entries, the import process recognizes and maps those entries to the Fabric pre-deployment and post-deployment configuration. Because Fabric supports only one pre-deployment and one post-deployment script, ensure the project contains at most one entry of each type before importing.
