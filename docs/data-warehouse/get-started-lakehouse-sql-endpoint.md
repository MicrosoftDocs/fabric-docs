...
This article explains the data warehousing experience with the [!INCLUDE [fabric-dw](includes/fabric-dw.md)] of the Lakehouse, and scenarios for use of the Lakehouse in data warehousing.

[!INCLUDE [preview-note](../includes/preview-note.md)]

## What is a Lakehouse SQL Endpoint?

In Fabric, when you [create a lakehouse](../onelake/create-lakehouse-onelake.md), a [[!INCLUDE [fabric-dw](includes/fabric-dw.md)]](data-warehousing.md#sql-endpoint-of-the-lakehouse) is automatically created. 

The [!INCLUDE [fabric-dw](includes/fabric-dw.md)] enables you to query data in the Lakehouse using T-SQL language and TDS protocol. Every Lakehouse has one [!INCLUDE [fabric-dw](includes/fabric-dw.md)], and each workspace can have more than one Lakehouse. The number of [!INCLUDE [fabric-dw](includes/fabric-dw.md)]s in a workspace matches the number of Lakehouse items.

- The [!INCLUDE [fabric-dw](includes/fabric-dw.md)] is automatically generated for every Lakehouse and exposes Delta tables from the Lakehouse as SQL tables that can be queried using the T-SQL language.
...
There's no need to create a [!INCLUDE [fabric-dw](includes/fabric-dw.md)] in Microsoft Fabric. Microsoft Fabric users can't create a [!INCLUDE [fabric-dw](includes/fabric-dw.md)] in a workspace. A [!INCLUDE [fabric-dw](includes/fabric-dw.md)] is automatically created for every Lakehouse. To get a [!INCLUDE [fabric-dw](includes/fabric-dw.md)], [create a lakehouse](../onelake/create-lakehouse-onelake.md) and a [!INCLUDE [fabric-dw](includes/fabric-dw.md)] will be automatically created for the Lakehouse.

> [!NOTE]
> Behind the scenes, the [!INCLUDE [fabric-dw](includes/fabric-dw.md)] is using the same engine as the [Warehouse](data-warehousing.md#synapse-data-warehouse) to serve high performance, low latency SQL queries.

### Automatic Metadata Discovery

A seamless process reads the delta logs and from the files folder and ensures SQL metadata for tables, such as statistics, is always up to date. There's no user action needed, and no need to import, copy data, or set up infrastructure. For more information, see [Automatically generated schema in the [!INCLUDE [fabric-dw](includes/fabric-dw.md)]](data-warehousing.md#automatically-generated-schema-in-the-sql-endpoint-of-the-lakehouse).

## Scenarios the Lakehouse enables for data warehousing

In Fabric, we offer one warehouse.

The Lakehouse, with its [!INCLUDE [fabric-dw](includes/fabric-dw.md)], powered by the Warehouse, can simplify the traditional decision tree of batch, streaming, or lambda architecture patterns. Together with a warehouse, the lakehouse enables many additive analytics scenarios. This section explores how to leverage a Lakehouse together with a Warehouse for a best of breed analytics strategy.
...
A [!INCLUDE [fabric-dw](includes/fabric-dw.md)] can read data in the `/tables` folder within OneLake. Analysis is as simple as querying the [!INCLUDE [fabric-dw](includes/fabric-dw.md)] of the Lakehouse. Together with the Warehouse, you also get cross-database queries and the ability to seamless switch from read-only queries to building additional business logic on top of your OneLake data with Synapse Data Warehouse.
...
Any folder referenced using a shortcut can be analyzed from a [!INCLUDE [fabric-dw](includes/fabric-dw.md)] and a SQL table is created for the referenced dataset. The SQL table can be used to expose data in externally managed data lakes and enable analytics on them.
...
1. Switch to the [!INCLUDE [fabric-dw](includes/fabric-dw.md)] of the Lakehouse and find a SQL table that has a name that matches the shortcut name. This SQL table references the folder in ADLS/S3 folder.
1. Query the SQL table that references data in ADLS/S3. The table can be used as any other table in the [!INCLUDE [fabric-dw](includes/fabric-dw.md)]. You can join tables that reference data in different storage accounts.

> [!NOTE]
> If the SQL table is not immediately shown in the [!INCLUDE [fabric-dw](includes/fabric-dw.md)], you might need to wait a few minutes. The SQL table that references data in external storage account is created with a delay. 
...
A [!INCLUDE [fabric-dw](includes/fabric-dw.md)] enables you to leave the data in place and still analyze data in the Warehouse or Lakehouse, even in other Microsoft Fabric workspaces, via a seamless virtualization. Every Microsoft Fabric Lakehouse stores data in OneLake.
...
1. Switch to the [!INCLUDE [fabric-dw](includes/fabric-dw.md)] of the Lakehouse and find the SQL table that has a name that matches the shortcut name. This SQL table references the folder in another workspace.
1. Query the SQL table that references data in another workspace. The table can be used as any other table in the [!INCLUDE [fabric-dw](includes/fabric-dw.md)]. You can join the tables that reference data in different workspaces.

> [!NOTE]
> If the SQL table is not immediately shown in the [!INCLUDE [fabric-dw](includes/fabric-dw.md)], you might need to wait a few minutes. The SQL table that references data in another workspace is created with a delay.
