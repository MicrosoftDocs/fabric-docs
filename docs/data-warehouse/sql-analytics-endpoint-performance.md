...
- Not all Delta features are understood by the automatic sync process.  For more information on the functionality supported by each engine in Fabric, see [Delta Lake Interoperability](../get-started/delta-lake-interoperability.md).
- If there are an extremely large volumne of tables changes during the Extract Transform and Load (ETL) processing, this build of changes can cause a delay until all the changes are processed.
