...
The **Results** preview is displayed in the **Results** section. If number of rows returned are more than 10,000 rows, the preview is limited to 10,000 rows. You can search string within results grid to get filtered rows matching search criteria. The **Messages** tab shows SQL messages returned when SQL query is run.
...
- In SQL query editor, when you click **Run** button, you are submitting an independent batch request to execute.  SQL query editor does not support sp_set_session_context (Transact-SQL) - SQL Server Microsoft Learn as SQL Server Management Studio. You can set session_context for your single batch request.
