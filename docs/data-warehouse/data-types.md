...
\*\*\* Support for **varchar (max)** and **varbinary (max)** is currently in preview for the [!INCLUDE [fabric-dw](includes/fabric-dw.md)].
> [!NOTE]
> varchar(max) is not supported in the [!INCLUDE [fabric-dw](includes/fabric-se.md)]
