...
The warehouse measures the age of the retained data in absolute calendar days from the time the data version was created, including anytime the [!INCLUDE [product-name](../includes/product-name.md)] capacity is paused.
...
<a id="increasing-the-retention-period"></a>
...
While planning the retention configuration, consider the trade-off between the benefits of longer data history access and the associated storage costs. For more information on monitoring storage, see [Billing and utilization reporting in Fabric Data Warehouse](usage-reporting.md).
