...
> [!IMPORTANT]
> Access control in all other scenarios is unsupported. This includes cross-tenant scenarios, such as external data sharing, where data is accessed from another tenant, or other export paths, such as export to .csv files or .txt files. 
