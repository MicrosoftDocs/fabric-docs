...
- Business user: Visually explores relationships, runs natural language queries, and gains insights effortlessly.
...
- Create a labeled property graph over structured data in OneLake by defining node and edges.

- Query using Graph Query Language (GQL) semantics, including pattern matching, path constructs, aggregations, and additional features as they are released. The official International Standard for GQL is [ISO/IEC 39075 Information Technology - Database Languages - GQL](https://www.iso.org/standard/76120.html)

- Enrich your graph with scores and communities for downstream BI (business intelligence) and AI (artificial intelligence) workflows.

- Benefit from job function-based experiences:  
    - Data engineers can model and create the graph.  
    ...
    - Developers and data scientists can integrate graph analytics into their applications.
...
| Language | Graph in Microsoft Fabric is compatible with the GQL standard (preview). | Standalone graph databases often use vendor-specific query languages. |
...
Graph in Microsoft Fabric is rolling out to new Fabric regions every day. It is currently available in the following regions:
