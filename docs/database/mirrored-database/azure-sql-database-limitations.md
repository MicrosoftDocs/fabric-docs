...
- A table cannot be mirrored with the following attributes:
    ...
    - A table cannot be mirrored if the primary key is one of the data types: **sql_variant**, **timestamp/rowversion**, **datetime2(7)**, **datetimeoffset(7)**, **time(7)** where 7 is the 7 digit precision. Delta lake supports only 6 digit precision.
