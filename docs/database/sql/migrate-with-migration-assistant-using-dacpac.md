...
- A Fabric SQL connection created and enabled for gateway usage. Migration copy operations run through a gateway runtime. If gateway usage isn't enabled on the Fabric SQL connection, data copy operations fail.
...
- Schema analysis starts automatically.
...
1. Select **5. Copy data** in Migration Assistant.
...
### Step 6: Finalize copy

The **6. Finalize Copy** step completes the data migration and enables remaining items such as constraints, triggers, and indexes, to bring the database to the desired state.
