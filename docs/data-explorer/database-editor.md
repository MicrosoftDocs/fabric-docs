...
1. Select **New** on the ribbon then select the entry titled **Kusto Database**.
    ...
    :::image type="content" source="media/database-editor/create-database.png" alt-text="Screenshot of Kusto workspace that shows the dropdown menu of the ribbon button titled New. Both the New tab and the entry titled Kusto Databse are highlighted":::
     ...
     ![title.](media/database-editor/create-database.png)
...
1. Enter your database name, then select **Create**. You can use alphanumeric characters, underscores, periods, and hyphens. Special characters aren't supported.
    ...
    :::image type="content" source="media/database-editor/new-database.png" alt-text="":::
...
* Files.
* Blob container.
* Event Hub. For more information on how to bring data using Event Hub, see [Event Hub- Link TBD](LinkTBD).
...
1. Azure blob schema
    ...
    :::image type="content" source="media/database-editor/azure-blob-schema.png" alt-tex="Azure blob schema.":::
...
Ingestion can be done as a one-time operation, or as a continuous method using Event Hub. To establish a continuous data connection, see [Event Hub- Link TBD](LinkTBD).
