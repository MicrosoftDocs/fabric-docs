...
- Calculated columns and calculated tables that reference columns or tables in Direct Lake storage mode aren't supported. Calculation groups, what-if parameters, and field parameters, which implicitly create calculated tables, and calculated tables that do not reference Direct Lake tables are supported. 
...
- Auto data/time intelligence in Power BI is not supported. Marking your own date table as a date table is supported.
