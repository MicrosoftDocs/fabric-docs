...
To support the collaborative and scalable needs of creators, Dataflow Gen2 in Fabric introduces [fast copy](dataflows-gen2-fast-copy.md), which enables efficient ingestion of large data volumes by using Fabric's backend infrastructure to [store and process](data-in-staging-items.md) intermediate data during transformation. It can handle terabytes of data seamlessly. Dataflow creators can specify [data destinations](dataflow-gen2-data-destinations-and-managed-settings.md) for their transformed data, such as a Fabric lakehouse, warehouse, eventhouse, or Azure SQL Database, facilitating better data management and accessibility. And what's more, the recent integration of generative AI through [Copilot](copilot-fabric-data-factory.md) enhances the data preparation experience by providing intelligent code generation and automating repetitive tasks, providing an easier and faster path to create complex solutions.
...
| [Copilot for Data Factory](copilot-fabric-data-factory.md), which provides intelligent code generation to transform data with ease, and generates code explanations to help better understand complex tasks | No | Yes |
...
- **Premium customers with Fabric access:** Dataflow Gen2 is the recommended path, offering improvements in performance, scale, reliability, functionality, and built-in AI. Many teams find it helpful to start by evaluating Gen2 for new or evolving workloads where these benefits can be realized quickly.
- **Pro and Premium Per User (PPU) customers:** Dataflow Gen1 remains a supported option where it best fits your current needs. As new Dataflow Gen2 paths for Pro/PPU scenarios become available, we'll share clear guidance and recommended steps to help with a smooth transition.
- **GCC customers on Premium:** Dataflow Gen2 support in GCC environments is planned to be available before any future Gen1 transition milestones, ensuring a supported upgrade path.
