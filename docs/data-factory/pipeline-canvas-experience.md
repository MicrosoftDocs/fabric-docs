...
Each activity on the canvas appears as a node that shows the activity type, name, and common actions.
...
You connect activities by using dependency lines that define execution flow. Each dependency has a condition that determines when the downstream activity runs:
...
To create a dependency, select the connection button on a node and drag it to another activity, or use the connection icon on the node's action bar.
...
- **Variables** - Create variables that you can set and modify during pipeline execution.
...
- **Clearer node layout** - Activity nodes surface key information and actions directly on the canvas, so you can easily scan activity names and types.
...
- **Enhanced navigation** - Navigation controls and canvas performance are optimized for large pipelines, so you can stay oriented as you work with deeply nested or wide-spanning workflows.
