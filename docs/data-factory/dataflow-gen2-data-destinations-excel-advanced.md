...
- **SheetData**: A worksheet containing tabular data. This part is a *data-bound*. SheetData parts must contain inline data and can't use table references.
- **Chart**: A chart visualization. Can be placed on its own sheet or combined with data. This part is *data-bound* when it contains inline data, or it can reference data from a SheetData part.

> [!NOTE]
> *Data-bound parts* are parts that contain or reference tabular data. Both `SheetData` and `Chart` (with inline data) are data-bound parts. Each data-bound part registers its data source using the part's `Name`, so all data-bound parts must have unique names.
...
| 10954 | Duplicate container '{name}'. Each document part (row) needs a unique value in the Container column. | Two parts have the same `Sheet` value, which isn't allowed. Each worksheet can only be created once. Give each part a unique `Sheet` name, or remove the optional `Sheet` column to autogenerate names. |
| 10955 | Duplicate name '{name}'. Each document part (row) needs a unique Name. | Two data-bound parts (SheetData or Chart with inline data) have the same `Name` value. Each data-bound part must have a unique name because it registers a data source that other parts can reference. Rename one of the parts to resolve this error. |
| 10956 | Referenced table '{name}' wasn't found. | When using table references (empty tables with `Name` metadata), ensure the referenced `Name` matches a SheetData part's `Name` value. Check for typos and case sensitivity. |
...
| 10986 | Parts of type '{type}' in '{format}' files must have a Table as Data. TableReference isn't allowed. | SheetData parts must contain inline table data and can't use table references. Only Chart parts can reference data from other parts. Provide the actual table data directly in the `Data` column. |
| 10987 | Parts of type '{type}' must have a Table or a Table reference as Data. | Chart and SheetData parts require either a table or a valid table reference in the `Data` column. Ensure you're providing a table value, not null or another type. |
| 10989 | Part type '{type}' isn't supported. | Use a valid part type: `Workbook`, `SheetData`, or `Chart`. Check for typos in the `PartType` column. |
...
type table [
    ...
    PartType = nullable text,     // "Workbook", "SheetData", or "Chart"
...
**Cause**: The `Name` metadata on the reference doesn't match any SheetData part.

**Solution**: Ensure the metadata name matches exactly:

```powerquery-m
let
    // SheetData part with name "SalesData"
