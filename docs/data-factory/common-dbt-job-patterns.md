...
There isn't a single correct medallion design. Bronze, Silver, and Gold can use warehouses, lakehouses, or both. The right choice depends on where data lands, which engine should execute transformations, how curated data is served, and whether dbt runs independently or as part of a larger Fabric pipeline.
...
- The lakehouse SQL analytics endpoint is read-only and isn't used to write dbt model results. If your transformations require T-SQL execution, use the warehouse adapter instead.
- The lakehouse and warehouse adapters use different execution engines and support different capabilities.
...
- Account for runtime behavior. dbt job runtime V1.0 doesn't support build caching or item reuse. Each run compiles and executes the project from source. For large projects, include the full compilation and execution time when estimating scheduling windows and SLAs.

## Related content

- [dbt job in Fabric (preview)](dbt-job-overview.md)
