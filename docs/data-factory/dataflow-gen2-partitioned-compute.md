...
- **Billing**: You're billed for the dataflow run based on capacity unit (CU) consumption. Running partitions in parallel shortens the overall (wall-clock) time to complete the run, while Dataflow Gen2 calculates CU consumption from the combined compute of every partition, where each partition is charged for its own processing duration. As a result, a partitioned run can consume a similar or greater amount of CU than a sequential run of the same workload, even though it finishes in less time. For the full rate model, see [Dataflow Gen2 pricing](pricing-dataflows-gen2.md).
