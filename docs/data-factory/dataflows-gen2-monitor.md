...
To download the detailed logs for dataflows that are refreshed using a gateway, you need to enable the [Admin consent for gateway diagnostics](/data-integration/gateway/service-gateway-log-files) feature first, both on the tenant level and on the gateway level.
