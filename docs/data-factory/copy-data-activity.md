...
| **Storage path** | Specify the path that you want to contain the staged data. If you do not provide a path, the service creates a container to store temporary data. Specify a path only if you use Storage with a shared access signature or specify the storage integration for Snowflake, or you require temporary data to be in a specific location. |
