...
- **Partition option**: Specify the data partitioning options used to load data from SQL MI. Allowed values are: **None** (default), **Physical partitions of table**, and **Dynamic range**. When a partition option is enabled (that is, not **None**), the degree of parallelism to concurrently load data from an SQL MI is controlled by **Degree of copy parallelism** in copy activity settings tab.
