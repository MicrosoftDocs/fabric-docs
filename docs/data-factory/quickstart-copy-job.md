...
1. On the **Choose data source** page of the Copy job wizard, select your Fabric Warehouse.
...
1. On the **Choose data** page, select the source Warehouse table that includes the incremental column. Select **Next**.
...
1. On the **Choose data destination** page, select **Lakehouse** under **New Fabric item.**
...
1. On the **Settings** step, choose **Incremental copy** as the Copy Job mode. Select the column that serves as the incremental column. For the sample table, that's **ModifiedDate**.
...
1. After the Copy job item is saved successfully, it starts the first run to bring in the initial data from the source table.
...
- [CI/CD for Copy job in Data Factory in Microsoft Fabric](cicd-copy-job.md)
