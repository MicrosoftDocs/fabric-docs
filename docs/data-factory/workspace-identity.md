...
- The identity that runs the item must have an admin, member, or contributor role in the workspace. Fabric checks this permission at run time and issues the workspace identity token only to an identity that holds one of these roles. A viewer role isn't sufficient. This requirement applies both to the user who runs a pipeline on demand and to the service principal that runs it on a schedule or through the API. If a scheduled run fails but a manual run of the same pipeline succeeds, verify the workspace role of the scheduled identity.
