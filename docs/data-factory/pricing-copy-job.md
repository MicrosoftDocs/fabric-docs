...
- Full copy: CUs are charged when you perform a full data load from source to destination using Copy job. This can be a one-time full copy or a recurring full snapshot reload.
- Incremental copy: CUs are charged when you use Copy job's incremental copy mode. This rate applies to both the initial full load and the subsequent runs that move only new or changed data. For databases, this means only new or updated rows are transferred, and if Change Data Capture (CDC) is enabled, inserts, updates, and deletes are also captured. For storage sources, only files with a newer LastModifiedTime are copied.
