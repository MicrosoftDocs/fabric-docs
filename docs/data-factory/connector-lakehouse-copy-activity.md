...
|**File path** |The path to a folder/file under source data store. Apply when choosing **File path** in **File path type**.| \<file path>|Yes when choosing **File path**|• folderPath<br>• fileName |
...
|**File path**|The path to a folder/file under destination data store.|\<file path>|No|• folderPath<br>• fileName|
