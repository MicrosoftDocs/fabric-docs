...
| *Entity type details* | The view in the ontology (preview) item where you can view and explore your instantiated ontology data. The experience includes basic data previews, instance data, and a graph view. |
| *[Graph in Microsoft Fabric](../../graph/overview.md)* | A Fabric item that offers native graph storage and compute for nodes, edges, and traversals over connected data. It's good for path finding, dependency analysis, and graph algorithms. When you create an ontology item, a managed Graph item is created automatically. That Graph is integrated into ontology's [entity type details](how-to-view-entity-type-details.md) and can be accessed independently in the Fabric workspace where the ontology item is located. |
