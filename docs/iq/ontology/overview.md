...
>[!IMPORTANT]
>Ontology's graph feature relies on [Graph in Microsoft Fabric](../../graph/overview.md), so the Graph setting must be enabled for your tenant. For more information, see [Prepare your tenant for ontology (preview)](overview-prepare-tenant.md).
...
Prepare your tenant for ontology (preview) by enabling required tenant settings in [Prepare your tenant for ontology (preview)](overview-prepare-tenant.md).