...
Use data input rows and columns to extend the Planning sheet by entering values directly within the matrix.You can capture business inputs that aren't available in the underlying dataset or need manual adjustments while maintaining the sheet's hierarchy, totals, and calculations.These inputs support planning, forecasting, and operational scenarios where certain values must be captured manually or adjusted within the sheet.
...
* Enable business users to enter data directly in the Planning sheet.
