...
1. Select a cell/row/column. Select **Comment**, then select **Add New Comment**.
...
* After a task is assigned, the assignee can respond in the comment thread. If the response resolves the issue, you can close the thread. Click **Resolve Thread**.
...
* Click the undo icon or reply to a thread to reopen it.
...
* Comments can be locked to prevent further editing and replies. Click **Lock Thread**.
...
* Click the unlock icon to make the thread editable again.
...
## View and manage comments in a report
...
* Click the refresh icon to update the report with the latest comments.
