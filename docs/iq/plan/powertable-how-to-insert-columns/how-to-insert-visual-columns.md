...
The visual column type includes multi-select relationship columns, reference columns, relation columns, rollup, button, and attachment columns. Visual columns exist only within the PowerTable sheet. PowerTable doesn't add them to the source database.
...
| **Relation** | Each record provides a navigation link to related child records in another table (master-detail relationship). | Open all order records for a selected customer or view all tasks related to a project. For more information, see [Insert relation columns in PowerTable](how-to-insert-relation-columns.md). |
| **Rollup** | Aggregates values from related records and shows a summarized result, such as a sum, count, average, minimum, or maximum value. | Show total project cost, total hours worked, or the number of open tickets for a customer. For more information, see [Insert rollup columns in PowerTable](how-to-insert-rollup-columns.md). |
