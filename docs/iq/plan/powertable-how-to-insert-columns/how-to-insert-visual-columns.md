...
| **Reference** | References and shows read-only values from another table based on matching keys. | Show a customer's region, department name, or manager name without duplicating data. |
...
| **Button** | Adds an action button that opens a URL or triggers an automation. | Open a URL, launch an automation, or start a workflow. |
| **Attachment** | Stores and manages files associated with a record. | Attach contracts, invoices, images, or supporting documents. |
