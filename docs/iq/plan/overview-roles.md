...
> [!NOTE]
> Creating a new plan workload automatically upgrades your session to Planner. Because workload creation requires Planner capabilities, no warning or confirmation prompt appears.
...
**Session expiry:** Each session automatically expires after 30 days. After the 30-day session expires, a new session begins only when you perform a new action on a plan item. The first successful action determines the persona for the new session:
   ...
   * If you perform a Planner-level action (for example, creating a new planning sheet or write back data), the new session starts as a Planner session. Each new session inherits its role from your first successful activity.
