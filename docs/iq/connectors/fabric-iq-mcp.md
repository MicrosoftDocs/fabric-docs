...
- Large `ExecuteQuery` results are returned as an embedded CSV resource with an inline preview. The calling MCP client must support embedded resources and make the CSV available to its agent; otherwise, the agent might see only the preview. Client support varies. Use aggregation, filters, and focused queries to reduce result size. CSV delivery remains subject to Power BI query limits, so results might be truncated.
