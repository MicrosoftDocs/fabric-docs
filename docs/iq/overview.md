...
* **AI readiness and decision-ready actions:** Provides structured grounding for copilots and agents, so answers reflect your enterprise language as defined in your ontology. Because business rules and constraints live in the ontology, agents can move beyond answers to safe, auditable actions. Ontology rules express condition–action logic on business entities and their properties, and Fabric Activator evaluates these rules to trigger governed, real-time actions (for example, alerts or notifications) when conditions are met.
...
* *[Ontology (preview)](ontology/overview.md)* is an item for the enterprise vocabulary and semantic layer that unifies meaning across domains and OneLake sources. It defines entity types, relationships, properties, and condition–action rules aligned to business entities and properties. These rules are executed by Fabric Activator to trigger actions across Fabric. The ontology binds all of these definitions to real data so that downstream tools share the same language. Ontologies are the core item for defining a common language in the IQ workload.
