...
Business events, Fabric events, and Azure events use at-least-once delivery. The system retries delivery for up to 24 hours from the time the source successfully generates the event. In rare cases, the system delivers the same event more than once. Events also **aren't guaranteed to arrive in order**, so your application shouldn't depend on the sequence in which events are received. Because delivery uses at-least-once, design your event consumers to handle receiving the same event more than once. The next section explains how to do this.
