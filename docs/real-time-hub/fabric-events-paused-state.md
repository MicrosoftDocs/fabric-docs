...
| Event ingestion is paused because the network configuration for source workspace blocks events to destination workspace. | The source workspace (the workspace where the events originate) has [workspace-level private links](/fabric/security/security-workspace-level-private-links-overview) configured to block public access, and there's no private link established from the consumer's network to the source workspace. This can also occur when [outbound access protection](/fabric/security/workspace-outbound-access-protection-real-time-events) is enabled on the consumer's workspace without an allowed Real-Time Events connector. | For workspace-level private links: allow public access on the source workspace, or establish a private link from the consumer's network to the source workspace. For outbound access protection: add the **Real-Time Events** connector to the consumer workspace's data connection rules. For more information, see [Private links for Azure and Fabric Events](private-links-real-time-events.md) and [Workspace outbound access protection for Real-Time Events](/fabric/security/workspace-outbound-access-protection-real-time-events). |
...
- [Workspace outbound access protection for Real-Time Events](/fabric/security/workspace-outbound-access-protection-real-time-events)
