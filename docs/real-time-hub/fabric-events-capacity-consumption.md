...
### Event listener 
This meter kicks in when a consumer is created for Fabric events and is charged to the consumer’s capacity. For example when an Activator trigger is created to act on an event, this meter is charged per hour during the existence of this trigger.
