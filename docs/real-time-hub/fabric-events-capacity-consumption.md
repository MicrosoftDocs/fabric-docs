---
title: Azure, Fabric, and Business events capacity consumption
description: Learn how to monitor capacity consumption for Azure, Fabric, and Business events.
...
# Azure, Fabric, and Business events capacity consumption

Azure, Fabric, and Business events support event-driven scenarios like real-time alerting and triggering downstream actions. Events are emitted by publishers and received by consumers like Activator or Eventstreams to enable you to monitor and react to changes by triggering actions or workflows.

Azure and Fabric events are published by sources like Azure Storage accounts or Fabric Workspaces when changes occur in those resources. Business events are published by Fabric items such as Notebooks or User Data Functions, and routed through an EventSchemaSet to consumers like Activator.
...
## Operation types
Azure, Fabric, and Business events usage is defined by two operation types, which are described in the following table. The table provides information about event operations shown in the Fabric Capacity Metrics app and their Fabric consumption rates. For more information about the app, see [Microsoft Fabric Capacity Metrics app](../enterprise/metrics-app.md).
...
### Event listener 
This meter kicks in when a consumer is created for Azure, Fabric, or Business events and is charged to the consumer's capacity. For example when an [!INCLUDE [fabric-activator](../real-time-intelligence/includes/fabric-activator.md)] rule is created to act on an event, this meter is charged per hour during the existence of this rule.

### Azure, Fabric, and Business events operations in the Microsoft Fabric Capacity Metrics app
...
> [!NOTE]
> When an Activator rule is created to take an action on an event, the Event Operations for delivery and filtering as well as the Event Listener operation show the metadata of the Activator artifact.
