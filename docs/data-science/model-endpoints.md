...
You can activate, configure, and query model endpoints by using a [public-facing REST API](https://aka.ms/fabric/model-endpoint-api). You can also get started directly from the Fabric interface by using a low-code experience to activate model endpoints and preview predictions instantly.
...
| **Auto sleep** | This property (`On` or `Off`) indicates whether the endpoint, once active, should scale down capacity usage to zero in the absence of traffic. If auto sleep is on, the endpoint enters an idle state after five minutes without incoming requests. The first call to wake up an idle endpoint involves a short delay. | `On` |
...
| **Status** | **Description** |
|---|---|
| `Inactive` | The endpoint isn't activated to serve real-time predictions, and it isn't consuming Fabric capacity. |
...
| `Deactivating` | The endpoint is being deactivated, so it no longer serves real-time predictions or consumes Fabric capacity. Behind the scenes, Fabric dismantles the underlying container infrastructure. |
...
- Manage and query endpoints programmatically by using the [ML model endpoint REST API](https://aka.ms/fabric/model-endpoint-api).
...
- Generate batch predictions by using the [`PREDICT` function](./model-scoring-predict.md) in Fabric notebooks.
