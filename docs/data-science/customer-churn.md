...
In this tutorial, you see a Microsoft Fabric data science workflow with an end-to-end example. The scenario is to build a model to predict the churn rate for bank customers. The churn rate, also called the rate of attrition, is the rate at which customers stop doing business with the bank.
...
- Upload a notebook from GitHub to the Data Science experience.
...
Out of the 10,000 customers, only 2,037 customers (around 20%) have left the bank. Therefore, given the class imbalance ratio, we recommend generating synthetic data. Moreover, confusion matrix accuracy might not be meaningful for imbalanced classification and it might be better to also measure the accuracy using the Area Under the Precision-Recall Curve (AUPRC).
...
Here you'll load the delta table from the lakehouse. You can use other delta tables considering the lakehouse as the source.
