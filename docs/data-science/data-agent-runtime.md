...
The runtime you choose determines how and when changes to the agent's core components reach your agent. It doesn't determine which data sources you can add. Some preview configurations, such as schema object descriptions and topics, are available only on the preview runtime.
...
| Topic-based context for SQL data sources | [Topics (Preview)](data-agent-topics.md) | Add up to 1 million characters of instructions organized by topic. The data agent finds the sections relevant to a question and provides them to NL2SQL. |
| Newer model for SQL query generation | [Advanced NL2SQL](data-agent-sql-sources.md#advanced-nl2sql-preview) | Advanced NL2SQL runs on a newer model to improve query accuracy and reduce latency. |
