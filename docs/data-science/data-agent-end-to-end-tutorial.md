...
```
> [!IMPORTANT]
> The code below uses the OpenAI Assistants API (`beta.assistants`, `beta.threads`, `beta.threads.runs`), which [OpenAI has deprecated with a shutdown date of August 26, 2026](https://platform.openai.com/docs/assistants/migration). The current code continues to work until **August 26, 2026** but plan for migrating to [MCP endpoint](https://learn.microsoft.com/fabric/data-science/data-agent-mcp-server) before this date.
