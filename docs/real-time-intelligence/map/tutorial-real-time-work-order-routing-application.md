...
Before starting this tutorial, review the [Real-Time Intelligence tutorials](/fabric/real-time-intelligence/tutorial-introduction) to become familiar with the core concepts and workflows.
...
* A basic understanding of [Kusto user-defined functions](/kusto/query/functions/?view=microsoft-fabric&preserve-view=true).
...
The work order data file contains sample work order records used in this tutorial to simulate a real‑time streaming source. After creating the file, import it into a lakehouse in the next step.

Copy and paste the following content into a text file, and then save it as *WorkorderLocations.csv*. You use this file in the next step.
...
Create a new lakehouse for incoming work order data and import the work order location file.

1. From your workspace, select **New item**, enter *lakehouse* in the search box, and select it to create a new lakehouse.
1. Enter a name *WorkorderLocationsLakehouse* and select **Create**.
1. In the new lakehouse, select **Upload files** and upload the *WorkorderLocations.csv* file.
...
Microsoft Fabric Eventstream is a real-time data streaming service that enables you to ingest, process, and route event data within the Microsoft Fabric ecosystem. It provides a no-code experience for building event-driven workflows, allowing seamless integration of real-time data from various sources and routing it to multiple destinations. For more information about supported data sources or how to connect to a custom endpoint, see the [Overview of Microsoft Fabric eventstreams](/fabric/real-time-intelligence/event-streams/overview).

By ingesting eventstream data into an eventhouse, you make streaming events available for processing with Kusto, where you can transform and analyze them in real time using tables or functions. For more information, see [Eventhouse overview](/fabric/real-time-intelligence/eventhouse).
...
1. In the **New Eventstream** dialog, enter a **Name**: "WorkordersEventstream", and then select **Create**.
...
1. The **Eventhouse destination configuration** pane appears on the right side of the screen. Fill out the details requested as follows, then select **Save**:
    ...
    1. **Activate ingestion after adding the data source**: Select the checkbox.
...
In this section, you create a notebook connected to the lakehouse you created earlier, then use the provided code to read the CSV data and send events to the eventstream. This process simulates real‑time data ingestion. For demos, you can run the notebook manually or schedule it to run periodically.

### Create a notebook in your Fabric workspace

Create a notebook with code to import the work order location file from your lakehouse into the eventstream you created in the previous section. This simulates a real-time streaming source, which in a production environment streams data rather than using static data.
...
1. Select **WorkorderLocationsLakehouse** from the **OneLake catalog** and select the **Connect** button. This lakehouse is the one you created [previously](#create-a-lakehouse-and-import-the-work-order-data-file).

1. After creating the notebook and connecting it to your lakehouse, paste the following code into the first cell and run it to install the **Azure Event Hubs** SDK:
...
1. Select **+ Code** to create a new cell in the notebook.
    ...
    :::image type="content" source="media/tutorials/real-time-work-order-routing-application/add-code-cell.png" alt-text="A screenshot of the Microsoft Fabric notebook interface showing a code cell with two lines of Python code. Line 1 contains a comment reading Install Azure Event Hubs SDK only needed once per environment. Line 2 shows the pip install command percent pip install azure-eventhub. Below the code cell, a tooltip displays Add code cell pointing to a plus Code button highlighted with a red rectangle. A plus Markdown button appears to the right. The upper right corner shows PySpark Python as the selected kernel.":::
...
1. Add the values for the variables **CONNECTION_STR** and **EVENT_HUB_NAME** that you got in the previous section titled [Get required SAS key authentication keys](#get-required-sas-key-authentication-keys).
...
1. In the **New item** pane, enter **map** into the search field, and select **Map**.
...
1. Select **KQL database** from the menu that appears when selecting the **Add** button.

    :::image type="content" source="media/tutorials/real-time-work-order-routing-application/add-eventhouse-map.png" alt-text="A screenshot showing the Microsoft Fabric Maps interface with the Explorer pane on the left with Fabric items and external sources tabs. The Fabric items tab is selected with an add button below it, with a drop-down menu below that, which appeared after selecting the add button. It has KQL database selected. The main area displays the default world map in the map area.":::

1. From the **OneLake** catalog, select the eventhouse **WorkordersEventhouse** that you created previously, then select **Add**.

> [!TIP]
> If you get an error such as *The KQL database has a protected label that restricts access. Please contact your database owner for assistance.*, check the sensitivity label on your KQL database, as it can restrict access. For more information, see [Apply sensitivity labels to Fabric items](/fabric/fundamentals/apply-sensitivity-labels).
...
1. Go to the KQL function *WorkordersFunction*, and select the ellipse (**...**) to open the menu.
1. Select **Show on map** from the popup menu.

    :::image type="content" source="media/tutorials/real-time-work-order-routing-application/show-on-map.png" alt-text="A screenshot of the Microsoft Fabric Maps Explorer panel displaying a hierarchical tree structure with KQL database section expanded. The tree shows WorkordersEventhouse containing Workorders-Event table and WorkordersFunction function. An ellipsis menu is open next to WorkordersFunction revealing options including Show on map highlighted with a red rectangle border.":::
...
1. In the **Set geometry and data refresh interval** step, set the fields as follows, then select **Next**:
    ...
    * **Geometry column location**: Latitude and longitude data are in separate columns
    * **Latitude column**: Latitude
    * **Longitude column**: Longitude
    * **Data refresh interval**: 5 minutes

        :::image type="content" source="media/tutorials/real-time-work-order-routing-application/set-geometry.png" alt-text="A screenshot of the Microsoft Fabric dialog titled View Eventhouse data on map showing the Set geometry and data refresh interval configuration step. A left sidebar displays three workflow steps with checkmarks indicating Preview data is completed and Set geometry and data refresh interval is currently active. The main panel contains a Data layer section with a Name field containing WorkordersFunction, followed by a Geometry data column section with three dropdowns: Geometry column location set to Latitude and longitude data located on separate columns, Latitude column set to Latitude, and Longitude column set to Longitude. Below is a Data refresh section with a Data refresh interval dropdown set to 5 minutes. Back and Next buttons appear at the bottom right corner.":::

1. In the **Review and add to map** step, review the settings and select **Add to map**.

    :::image type="content" source="media/tutorials/real-time-work-order-routing-application/review-add-to-map.png" alt-text="A screenshot of the Microsoft Fabric View Eventhouse data on map dialog showing the final Review and add to map step of the configuration workflow. The left sidebar displays three sequential workflow steps: Preview data and Set geometry and data refresh interval both completed with green checkmarks, and Review and add to map currently active with a blue dot indicator. The main panel shows the summary of configuration settings: Data source section lists KQL database as WorkordersEventhouse and queryset as WorkordersFunction. The Data layer section displays the layer name WorkordersFunction marked as required with a red asterisk. The Geometry data column section shows three required fields marked with red asterisks: Geometry column location set to Latitude and longitude data located on separate columns, Latitude column set to Latitude, and Longitude column set to Longitude. The Data refresh section indicates the refresh interval is set to 5 minutes, also marked as required. The dialog footer contains Back and Add to map buttons positioned at the bottom right, with the Add to map button highlighted ready to finalize adding the eventhouse data layer to the map.":::

The function results now appear in the updated map.
...
1. The top menu bar now displays an option for **Notebook**. Select it to create a new notebook.
...
1. In the new notebook, save the value for the **kustoUri** variable. You use this value in the new notebook code you create in step 6.
1. Connect your notebook to **WorkorderLocationsLakehouse** by selecting **From OneLake catalog** from the **Add data items** dropdown list.
    ...
    >
    > When attaching or creating a notebook, ensure you select the **Lakehouse**, not the SQL analytics endpoint. If **WorkorderLocationsLakehouse** appears twice in the **OneLake catalog**, filter by **Lakehouse** .

1. Once your new notebook is created and connected to your lakehouse, enter the following code into the second cell of your notebook, replacing the default code. Then add the variable values you saved in the previous step:
...
1. Locate the **Labels** checkbox, and uncheck it.
...
Data labels are data‑driven annotations that come from one or more fields in the layer's dataset. They're tied directly to layer level map features, such as the points on the map that represent work order locations. For more information about Fabric Maps data labels, see [Data label settings](customize-map.md#data-label-settings).

1. Select **Optimized Route** in the **Data layers** pane. The **Optimized Route** settings pane appears in the right side of the screen.
1. In the **Optimized Route** settings pane, select **> Data label settings**.
