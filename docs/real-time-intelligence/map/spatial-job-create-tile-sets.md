...
Fabric Maps currently supports PMTiles as the format for its tilesets, which can be created using raster or vector tiles. It also allows generating PMTiles from valid GeoJSON files using vector tiles, as described in this article. For more information on using PMTiles in Fabric Maps, see [Add data to the map - PMTiles](create-map.md#add-data-to-the-map---pmtiles) in the *Create a map* article.
