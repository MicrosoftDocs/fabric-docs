...
- **Access control**:
    Permissions are enforced for both the map and its underlying data sources (like Lakehouse or eventhouse), ensuring secure, role-based access. Users don't need full workspace-level permissions.
- **User experience**:
...
| **Data Dependencies** | Recipients need access to underlying data | Same                                       |
...
- **How it works**:
    Maps are shared as part of an Org App, which is a packaged set of resources (maps, reports, datasets, etc.) published to your organization. Org Apps are discoverable and accessible to users across the organization through the Apps experience in Fabric.
- **Access control**:
    Permissions are managed at the app level. Users who have access to the Org App automatically get access to all included maps and resources, typically based on their organizational role or group membership.
- **User experience**:
...
| **Permissions**| Manual, per artifact                                | Automatic (for map; dependencies in future phases) |
