...
Start by creating an alert for a report visual. This example uses a report visual. 

1. Open your Power BI report in [editing view](/power-bi/create-reports/service-interact-with-a-report-in-editing-view).
1. Choose a visual on the report to monitor, in the upper right corner of the visual select the ellipses (...) > **Add alert**. You can also select the bell icon in the visual.
...
1. In the **Alerts** pane, at the bottom of the window, notice the **Activator** item associated with the alert at the bottom. An Activator item can have one or more alerts in it. You can use it group a set of related alerts in one Fabric item. If you want to change the Activator item associated with the report, follow these steps. If not, move to the next step. 

    To change the Activator item, select the **Activator** item (or) select **Ellipsis (...)**, and then choose **Select alert location**. In the **Select the location of your alerts** window, follow these steps:

    1. For **Workspace**, select the **Fabric workspace** where you want to save the Activator item with alerts. 
    1. For **Item**, select an existing Activator item, or select **Create a new activator item**. 
    1. If you select **Create a new activator item**, enter a name for the item in the **Item name** textbox.
    1. Then, select **Confirm**. 
    
        :::image type="content" source="media/activator-get-data-power-bi/alerts-location.png" alt-text="Screenshot showing the alerts location window." lightbox="media/activator-get-data-power-bi/alerts-location.png":::
    1. In the **Alerts** window, select **Add alert**. 
    
        :::image type="content" source="media/activator-get-data-power-bi/add-alert-button.png" alt-text="Screenshot showing the alerts location window with the Add alert button highlighted." lightbox="media/activator-get-data-power-bi/add-alert-button.png":::        
    1. Select the measure associated with the tile for which you want to create the alert. 
1. In the **Alert** window, you see two sections for the alert you're trying to create: **Condition** and **Send notification** with an action to be taken when the alert condition is met. 
...
1. In the **Send notification** or Action section, follow these steps:
    ...
    1. Select **Apply**. 
    1. Confirm that you see **1 alert on this report** at the top of the **Alerts** window.
...
## Add another alert 
In the previous example, you set up an alert using the Add alert option for a tile in the report. There's also a **Set alert** button on the ribbon or **Set alert** button in the **Alerts** window that you can use. When you use these options, it shows all the measures associated with the report, not the one associated with a particular tile. 

1. Select **Set alert** on the bottom. If you have the **Alerts** window, select **Add alert**. 
...
1. From the list of available measures in the report to set up an alert, select the **measure** you want to use to set up the alert. 
