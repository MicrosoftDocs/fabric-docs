...
1. You see that the Azure IoT Hub source is added to your eventstream on the canvas in the **Edit** mode. To implement this newly added IoT hub, select **Publish** on the ribbon.
...
1. After you complete these steps, the IoT hub is available for visualization in the **Live view**. Select the **IoT hub** tile in the diagram to see the page similar to the following one.
...
2. On the **Azure IoT Hub** configuration pane, enter the following details:
   ...
   :::image type="content" source="./media/add-iot-hub-source/iot-hub-configuration-pane.png" alt-text="Screenshot that shows where to configure Azure IoT Hub in the eventstream.":::
    ...
    1. **Cloud connection**: Select an existing cloud connection that links your IoT hub to Microsoft Fabric. If you don't have one, proceed to step 3 to create a new cloud connection.
    1. **Data format**. Choose a data format (AVRO, JSON, or CSV) for streaming your IoT hub data into the eventstream.
    1. **Consumer group**. Choose a consumer group from your IoT hub, or leave it as **$Default**. Then select **Add** to finish the configuration.
...
3. To create a new cloud connection for your IoT hub, follow these steps:

    1. Select **Create new** from the dropdown menu.
    ...
    1. Fill in the **Connection settings** and **Connection credentials** of your IoT hub, and then select **Create**.
    ...
    1. **IoT Hub**. Enter the name of the IoT hub in the Azure portal.

    1. **Connection name**. Enter a name for the new cloud connection, such as **iothub-connection**.

    1. **Shared access key name** and **Shared access key**. Enter the connection credentials for your IoT hub. You can find it under **Shared access policies** in the Azure portal. You must have appropriate permissions to access any of the Azure IoT Hub endpoints.
...
:::image type="content" source="./media/add-iot-hub-source/preview-iot-hub-data.png" alt-text="Screenshot that shows where to preview Azure IoT Hub data." lightbox="./media/add-iot-hub-source/shared-access-key.png":::
