...
If you're using **Eventhouse direct ingestion mode** destination, ensure that the `mappingRuleName` property is correctly specified. For detailed instructions on creating ingestion mappings, see [Mapping with ingestionMappingReference](/kusto/management/mappings?view=microsoft-fabric#mapping-with-ingestionmappingreference).
...
Again, if you're using **Eventhouse direct ingestion mode** destination, ensure that the `mappingRuleName` property is correctly specified. For detailed instructions on creating ingestion mappings, see [Mapping with ingestionMappingReference](/kusto/management/mappings?view=microsoft-fabric#mapping-with-ingestionmappingreference).
