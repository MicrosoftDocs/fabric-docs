---
title: MySQL Database CDC connector - prerequisites
description: This file has the prerequisites for using a MySQL Database Change Data Capture (CDC) connector for Fabric event streams and Real-Time hub. 
...
Currently, MySQL Database CDC is supported from the following services where the databases can be accessed publicly:
...
- Your MySQL database should be publicly accessible and not be behind a firewall or secured in a virtual network. If it resides in a protected network, connect to it by using [Eventstream connector virtual network injection](../../streaming-connector-private-network-support-guide.md).
...
> [!NOTE]
> The new user or admin account and the corresponding password is used to connect to database later inside Eventstream. 
...
1. Grant the required privileges to the user:
   ...
   > [!NOTE]
   > When a global read lock is unavailable, as in hosted options like Amazon Relational Database Service (RDS) or Aurora, table-level locks are used to create the consistent snapshot. In this case, you need to grant **`LOCK TABLES`** permission to the user. Additionally, to support **`FLUSH`** operations during the snapshot, you might also need to grant **`RELOAD`** or **`FLUSH_TABLES`** privileges.
...
1. On the **Server parameters** page, configure the following properties, and then select **Save**.
   ...
   :::image type="content" border="true" source="media/mysql-database-cdc-source-connector/binlog.png" alt-text="A screenshot of the binlog settings for replication under Server parameters." lightbox="media/mysql-database-cdc-source-connector/binlog.png":::
