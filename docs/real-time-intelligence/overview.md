...
- **Fabric events**: Event-driven capabilities support real-time notifications and data processing. You can monitor and react to events including Fabric Workspace Item events and Azure Blob Storage events. These events can be used to trigger other actions or workflows, such as invoking a pipeline or sending a notification via email. You can also send these events to other destinations via eventstreams. Fabric events and Azure Blob Storage events can also trigger Activator rules to initiate Spark jobs or Dataflows, enabling fully event-driven orchestration without schedules.
...
Digital twin builder data can also be connected to Power BI or Real-Time dashboards for additional visualization and customized reporting of your modeled data. Rules can also be defined on ontology business entities to initiate alerts and automated actions (preview), connecting modeled entities to downstream real-time actions.
...
alerting users, executing Fabric job items like a pipeline, Spark job, or
Dataflow, running User Data Functions, or kicking
