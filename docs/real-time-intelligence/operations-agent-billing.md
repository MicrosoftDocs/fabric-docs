...
- **Copilot in Fabric** is a dedicated operation that occurs when the large language model (LLM) directly configures or interacts with the operations agent. For example, you might use AI-driven suggestions or automation to set up, modify, or manage the operations agent's rules and actions.
...
Consumption rates can change at any time. Microsoft makes reasonable efforts to provide notice through email or in-product notification. Changes take effect on the date stated in release notes or in the Microsoft Fabric blog. If a change to a Microsoft Fabric workload consumption rate significantly increases the capacity units required to use a particular workload, you can use the cancellation options available for the chosen payment method.
