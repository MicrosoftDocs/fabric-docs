...
- The mirrored database item. This item enables downstream scenarios like data engineering, data science, and more. Mirroring manages:
  - Document Library data is surfaced in [OneLake](/fabric/onelake/onelake-overview) through shortcuts, which act as symbolic links to the underlying storage containing the Document Library tables, without replicating the data itself. These shortcuts allow Fabric to access the data directly, and supported data can be surfaced as Delta tables for downstream workloads.
