...
- Create or use an existing Azure Databricks workspace with Unity Catalog enabled.
- When ADLS Gen2 is behind a firewall, the workspace identity is required for trusted network access, regardless of the Databricks authentication method used. 
   ...
   - When ADLS Gen2 is protected by an Azure Storage firewall, Fabric uses Workspace Identity to access the storage account. Even if a service principal is selected for Databricks authentication, the Workspace Identity must be granted the appropriate permissions on the Azure Storage account.
