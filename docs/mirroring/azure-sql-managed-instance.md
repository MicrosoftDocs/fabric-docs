...
In addition to the [SQL query editor](../data-warehouse/sql-query-editor.md), there's an ecosystem of tooling that can query the SQL analytics endpoint, including [SQL Server Management Studio (SSMS)](/sql/ssms/download-sql-server-management-studio-ssms) and even GitHub Copilot.
