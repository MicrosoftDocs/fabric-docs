...
[Mirroring in Fabric](../mirroring/overview.md) provides an easy experience to avoid complex ETL (Extract Transform Load) and integrate your existing Snowflake warehouse data with the rest of your data in Microsoft Fabric. For supported sources and capabilities, see [Mirroring in Fabric (overview)](../mirroring/overview.md). You can continuously replicate your existing Snowflake data directly into Fabric's OneLake. Mirroring continuously replicates changes from Snowflake into OneLake as Delta Lake tables, enabling near real-time analytics without building ETL pipelines. Inside Fabric, you can unlock powerful business intelligence, artificial intelligence, Data Engineering, Data Science, and data sharing scenarios.
...
- The mirrored database item. This enables downstream scenarios like data engineering, data science, and more. Mirroring manages:
  ...
  - The replication of managed table data into [OneLake](../onelake/onelake-overview.md) as Delta Lake tables (stored in Parquet files) for use across Fabric workloads.
