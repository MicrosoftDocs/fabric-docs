...
When you work with Git‑connected workspaces in Fabric, you might encounter situations where items in your workspace and items in your Git branch have the same name and type, but different logical IDs. This mismatch triggers a metadata overwrite warning and requires you to decide which logical ID to keep.
...
Those IDs represent the "identity" of each item across Fabric workspaces. A logical ID is an automatically generated, cross‑workspace identifier that connects an item in a workspace with its corresponding item in a Git branch. Items with the same logical IDs are assumed to be the same. 
...
When a conflict is detected, you see a Confirm metadata overwrite dialog similar to the following:
...
- Workspace metadata now matches the Git version.
