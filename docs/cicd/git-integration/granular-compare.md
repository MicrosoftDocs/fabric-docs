...
Instead of only showing which items are affected, Compare code changes displays a side‑by‑side or inline diff of every modified file within each workspace item. This feature gives developers clarity, confidence, and control over the changes moving between the Fabric workspace and the connected Git branch.
...
In most cases, the changes in the system files are just like any other changes. If system platform files, upgrade from version 1 to version 2, you may see additions and deletions of files which won’t affect your current changes.
...
- If a conflict occurs, you can only open the compare dialog from the **changes** tab or via the conflict dialog.
