...
[Microsoft Fabric](https://www.microsoft.com/microsoft-fabric/blog/) offers three distinct pathways for independent software vendors (ISVs) to seamlessly integrate with Fabric. For an ISV starting on this journey, this article walks through various resources available under each of these pathways.
...
The primary focus of the Interop model is on enabling ISVs to integrate their solutions with the [OneLake Foundation](../../fundamentals/microsoft-fabric-overview.md). To interop with Fabric, Microsoft provides integration through a multitude of connectors in Data Factory and in Real-Time Intelligence. Microsoft also provides REST APIs for OneLake, shortcuts in OneLake, data sharing across Fabric tenants, and database mirroring.
...
- If you're using Databricks and want to connect to Fabric, Databricks works with ADLS Gen2 APIs. [Integrate OneLake with Azure Databricks](../../onelake/onelake-azure-databricks.md).
...
- You can ingest streaming data into eventstreams via multiple protocols including Kafka, Event Hubs, AMQP, and a growing list of connectors listed [here.](../../real-time-intelligence/event-streams/add-manage-eventstream-sources.md)
- After processing the ingested events using either the no-code experience or using SQL operator (Preview), you can route the result to several Fabric destinations or to custom endpoints. Learn more about eventstreams destinations [here.](../../real-time-intelligence/event-streams/add-manage-eventstream-destinations.md)
- **Eventhouses** are designed for streaming data, compatible with Real-Time hub, and ideal for time-based events. Data is automatically indexed and partitioned based on ingestion time, giving you incredibly fast and complex analytic querying capabilities on high-granularity data that you can access in OneLake for use across Fabric's suite of experiences. Eventhouses support existing eventhouse APIs and SDKs for direct interaction, allowing developers to read, write, and manage their data in eventhouses. Learn more about [REST API](/azure/data-explorer/kusto/api/rest/index?context=/fabric/context/context-rti&pivots=fabric).
...
- [Database shortcuts in Real-Time Intelligence](../../real-time-intelligence/database-shortcut.md) are embedded references within an eventhouse to a source database. The source database can either be a KQL database in Real-Time Intelligence or an Azure Data Explorer database. You can use shortcuts for in place sharing of data within the same tenant or across tenants. Learn more about managing [database shortcuts using the API](#develop-on-fabric).
...
- By using the supported Dataflow Gen2 connectors, ISVs can harness the power of Fabric Data Factory to manage complex data workflows. This feature is especially beneficial for ISVs looking to streamline data processing and transformation tasks. [Dataflow Gen2 connectors in Fabric](../../data-factory/dataflow-support.md).
- For a full list of capabilities supported by Data Factory, check out this [Data Factory Blog](https://blog.fabric.microsoft.com/blog/introducing-data-factory-in-microsoft-fabric?ft=All).
...
Mirroring in Fabric provides an easy experience to avoid complex ETL (Extract Transform Load) and integrate your existing data into OneLake with the rest of your data in Fabric. You can continuously replicate your existing data directly into OneLake. In Fabric, you can unlock powerful business intelligence, artificial intelligence, Data Engineering, Data Science, and data sharing scenarios. 
...
| Data Factory    | - [Data pipeline](/rest/api/fabric/datapipeline/items)<br> |
...
The [**Microsoft Fabric Workload Development Kit**](../../workload-development-kit/development-kit-overview.md) offers a comprehensive toolkit for developers to integrate applications into the Fabric hub. This integration allows for the addition of new capabilities directly within a Fabric workspace, enhancing the analytics journey for users. It provides developers and ISVs with a new avenue to reach customers, delivering both familiar and new experiences, and leveraging existing data applications. Fabric admins can manage who can add workloads in an organization.

### Workload Hub

The [Workload Hub](https://app.fabric.microsoft.com/workloadhub) in Fabric is a centralized interface where you can explore, manage, and access all available workloads. Each workload in Fabric is associated with a specific item type that you can create within Fabric workspaces. By navigating through the Workload Hub, you can easily discover and interact with various workloads, enhancing your analytical and operational capabilities.
...
- **[Neo4j AuraDB with Graph Analytics](https://app.fabric.microsoft.com/workloadhub/detail/Neo4j.GraphAnalytics.Neo4j%20Graph%20Analytics?experience=fabric-developer)**: Create graph models from OneLake data, visually analyze and explore data connections, query your data, and run any of the 65+ built-in algorithms with a seamless experience in Fabric.
...
- **[Power Designer](https://app.fabric.microsoft.com/workloadhub/detail/tips.tools.Product?experience=fabric-developer)**: A tool for company-wide styling and report template creation, improving Power BI report designs.
  ...
  :::image type="content" source="media/partner-integration/power-bi-tips-workload-tile.png" alt-text="Screenshot showing Power BI tips's workload." lightbox="media/partner-integration/power-bi-tips-workload-tile.png":::
  
  
- **[Celonis Process Intelligence](https://app.fabric.microsoft.com/workloadhub/detail/celonis.fabric-integration.Product?experience=fabric-developer)**: Allows organizations to expose Celonis’ unique class of data and context in Fabric.
...
- **[Quantexa Unify](https://app.fabric.microsoft.com/workloadhub/detail/Quantexa.Unify.Product?experience=fabric-developer)**: Enhances OneLake data sources by providing a 360-degree view with advanced data resolution capabilities.
...
- **[Teradata AI Unlimited](https://app.fabric.microsoft.com/workloadhub/detail/Teradata.AIUnlimited.AIUnlimited?experience=fabric-developer)**: Combines Teradata's analytic engine with Fabric's data management capabilities through Teradata's in-database functions.
...
- **[SQL2Fabric-Mirroring by Striim](https://app.fabric.microsoft.com/workloadhub/detail/striim.SQL2Fabric-Mirroring.Product?experience=fabric-developer)**: Fully managed, zero-code replication solution that seamlessly mirrors on-premises SQL Server data to OneLake.
