...
Understand how a Fabric app consumes Microsoft Fabric capacity and which platform features don't add separate charges. This article explains where capacity units (CUs) are used across the SQL database in Fabric, GraphQL API, and OneLake operations.

## How billing works

Fabric apps run on Microsoft Fabric capacity. Every operation performed by a Fabric app child service consumes CUs from the Fabric capacity assigned to your workspace.
...
- [Microsoft Fabric operations](../enterprise/fabric-operations.md)
