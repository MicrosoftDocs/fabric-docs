...
Capacity overage (preview) is an opt-in feature that automatically pays for excess capacity usage, up to a limit that the capacity admin sets. It prevents throttling and ensures that workloads continue uninterrupted, even when they temporarily exceed a capacity's limits.
...
Capacity overage prevents throttling by automatically paying off excess capacity usage up to a limit an admin sets. Here's how throttling in Fabric interacts with capacity overage:
...
Overage limits use Fabric quota, so you can only set a limit if it falls within your available quota. The required quota equals 1/24th of the limit you set, because Fabric spreads your CU hours limit across 24 hours. For example, a 48 CU hour limit adds 2 CUs to your quota. If the available quota can't support the configured limit, you can't enable capacity overage until you increase the quota or reduce the limit. To learn more about quotas, see [Fabric quotas](fabric-quotas.md).
...
| **Spending limit** | Set a 24‑hour CU hours limit. After you reach the limit, capacity overage stops and throttling resumes until usage rolls out of the window or you increase the limit. Capacity overage checks this limit every 5 minutes, so you might exceed your limit; consider this check when you set a reasonable limit. |
...
- **Adjusting limits:** You can update limits at any time. Increasing the limit resumes billing if overload persists. Lowering the limit might result in throttling if your processed capacity overages exceed the new limit.
