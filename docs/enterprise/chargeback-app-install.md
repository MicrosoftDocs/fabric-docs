---
title: Install the Chargeback app
description: Learn how to install the Microsoft Fabric Chargeback app from AppSource so you can distribute capacity costs based on actual usage across your organization.
...
#customer intent: As a Fabric capacity admin, I want to install the Microsoft Fabric Chargeback app so that I can distribute costs based on actual usage across my organization.
---

# Install the Microsoft Fabric Chargeback app

The Microsoft Fabric Chargeback app is a reporting app that helps you allocate Fabric capacity costs based on actual usage across your organization. This article shows you how to install the app from Microsoft AppSource. To learn more about what the app does, see [What is Microsoft Fabric Chargeback app?](chargeback-app.md)

## Prerequisites

- You must be an admin of at least one capacity in your organization. To learn how to check your admin status, see [Capacity settings](../admin/capacity-settings.md).
...
1. Select one of these options to get the app from AppSource:
    - Go to [AppSource > Microsoft Fabric Chargeback](https://go.microsoft.com/fwlink/p/?linkid=2320990) and select **Get it now**.
    - In Power BI service:
        1. Select **Apps**.
        1. Select **Get apps**.
        ...
        1. Select **Get it now**.
1. When prompted, sign in to AppSource by using your Microsoft account and complete the registration screen. The app takes you to Microsoft Fabric to complete the process. Select **Install** to continue.
...
1. In the **Update app** window, select one of the following:
    - **Update the workspace and the app**: This option updates your current app and the workspace it resides in. This option is the default for upgrading the app. It removes the current version and its workspace and replaces them with the new version of the app and a corresponding workspace.
    ...
    - **Install another copy of the app into a new workspace**: This option creates a new workspace and installs the new version of the app in this workspace. Select this option if you want to keep the old version of the app with the new one. This option creates another workspace for the new app version and installs the app in the newly created workspace. If you select this option, provide a name for the new workspace.
1. Select **Install**.
...
1. In the **Connect to Microsoft Fabric Chargeback** first window, fill in the fields according to the following table:
   ...
   | --------------------- | -------- | ------------------------------------------------------------------------ | ------------------------------------------------ |
   | **UTC Offset**        | Yes      |Numerical values ranging from `14` to `-12`.<br> To signify a half-hour time zone, use `.5`. For example, for Australian Central Standard Time, enter `9.5`.   |Enter your organization's standard time in Coordinated Universal Time (UTC). |
   | **Days Ago to Start** | Yes      | Choose between `14` or `30` days | Number of days of historical data to load. |
   |**Advanced**   |Optional |**On** or **Off** |The app automatically refreshes your data at midnight. To disable automatic refresh, expand the **Advanced** option and select **Off**. |
...
To confirm that the app installed and connected to your data successfully, open the *Microsoft Fabric Chargeback* app and confirm that your data appears in the report pages.
...
- [What is Microsoft Fabric Chargeback app?](chargeback-app.md)
