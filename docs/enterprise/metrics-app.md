...
- The Health page might fail to load if a user is a capacity admin in general but is using Just-In-Time (JIT) elevated tenant-level access and does not have admin permissions on the capacity in their home region. In such cases, updating the RegionName parameter in the semantic model settings (based on the user’s access level as described in the [installation](metrics-app-install.md) guidance) and refreshing the semantic model resolves the issue.
