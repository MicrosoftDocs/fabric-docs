...
When a client requests a user delegation key using an OAuth 2.0 token, OneLake returns a user delegation key on behalf of the client.  Any SAS signatures created with this user delegation key are granted at most the permissions granted to the client, scoped down to the permissions explicitly granted in the SAS.

You can create any number of OneLake SAS for the lifetime of the user delegation key. However, OneLake SAS and user delegation keys can be valid for at most one hour, and can't exceed the lifetime of the token requesting them.  These lifetime restrictions are different than those of Azure Storage user delegated SAS.  

## Construct a user delegation SAS

The following table summarizes the fields that are supported for a OneLake SAS token. Subsequent sections provide more details about these parameters, and how they differ from Azure Storage SAS tokens. OneLake doesn't support every optional SAS parameter supported by Azure Storage. OneLake SAS constructed with an unsupported parameter will be rejected.
...
|`signedPermissions`|`sp`|Required|Indicates which operations the SAS may perform on the resource. More details in the [Specify permissions](how-to-create-onelake-sas.md#specify-permissions) section|
