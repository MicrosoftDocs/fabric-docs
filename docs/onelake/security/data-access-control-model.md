...
When a user shares a lakehouse, they grant other users or a group of users access to the lakehouse without giving access to the workspace and the rest of its items. Shared lakehouses can be found through Data Hub or the **Shared with me** section in Microsoft Fabric.
...
|----------|----------|----------|----------|-----|
| *No additional permissions selected* | No by default. Use OneLake security to grant access. |  No | No | No |
| Read all Apache Spark | Yes by default. Use OneLake security to restrict access. |  No | No | No |
| Read all SQL endpoint data | No by default. Use OneLake security to grant access. |  No | Yes | No |
| Build  reports on the default dataset | Yes by default. Use OneLake security to restrict access. | No | No | Yes |
