...
> [!IMPORTANT]
> Only queries against KQL databases within an Eventhouse are supported. If your Queryset is connected to an [external ADX cluster](../real-time-intelligence/kusto-query-set?tabs=azure-data-explorer-cluster#select-a-database), creating an alert is not supported. 
