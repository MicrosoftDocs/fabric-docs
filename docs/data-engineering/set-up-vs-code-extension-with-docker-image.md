...
> [!NOTE]
> If you are using Windows Subsystem for Linux (WSL) backend for Docker, Windows manages resources limits. To increase resource limits, open the WSL Settings application or see [Advanced settings configuration in WSL](/windows/wsl/wsl-config#configure-global-options-with-wslconfig)
