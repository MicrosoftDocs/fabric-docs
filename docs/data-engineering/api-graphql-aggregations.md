...
## Aggregation querybBasics

To perform an aggregation, you typically specify a `groupBy` argument in your GraphQL query to define how to group the data, and request aggregation fields (like counts or sums) in the result. The response contains a list of grouped records, each with the group’s key values and the aggregated metrics.
...
This tells us category 1 has three products, category 2 has 2, and so on.

### Example 2: Sum and average 
You can request multiple aggregation metrics in one query. For example if you want, the total price of all products and the average rating for each category:
...
It's possible to filter the aggregated results with the `having` clause. For example, we can modify the previous example to only return results greater than two:
