...
For a step-by-step walkthrough with example code, see [Get started with the Livy API for Fabric High Concurrency Sessions](get-started-hc-livy.md). For a conceptual overview, see [High concurrency support in the Fabric Livy API](high-concurrency-livy.md).
