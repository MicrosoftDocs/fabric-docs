...
**Dataset**
Contoso has the raw data of its retail operations stored in CSV format at ADLS Gen2. Contoso hopes to leverage these to create the bronze layer of medallion architecture. It has created Spark SQL commands in a Notebook to create Fabric materialized views forming the slver and gold layers of the medallion architecture. 
...
1.	Once the materialized views for silver and gold layers are created, click on the ‘Managed Materialized View’ button in the Lakehouse to view the plain directed acyclic graph (DAG) autogenerated based on dependencies. You can find that that each dependent materialized views form the nodes of the DAG.
