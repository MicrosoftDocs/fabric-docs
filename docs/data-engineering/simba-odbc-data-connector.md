...
To install the Simba ODBC Data Connector for Microsoft Fabric Spark:
...
1. To change the installation location, select **Change**, then browse to the desired folder, and then select **OK**.
1. To accept the installation location, select **Next**.
1. Select **Install**.
...
Alternatively, you can specify connection settings in a connection string. Settings in the connection string take precedence over settings in the DSN. The following instructions describe how to create a DSN.
...
1. In the Authentication area, configure authentication as needed.**
1. To configure the connector to connect to Spark through a proxy server, select **Proxy Options**.
1. To configure client-server verification over SSL, select **SSL Options**.
1. To configure advanced connector options, select **Advanced Options**.
...
1. To configure two-way SSL verification, select the **Two-Way SSL** check box and then do the following:
    ...
    1. To encrypt your credentials, select **Password Options** and then select one of the following:
        ...
        1. Or, if the credentials are used by all users on the current Windows machine, select **All Users Of This Machine**.
    1. To confirm your choice and close the Password Options dialog box, select **OK**.
