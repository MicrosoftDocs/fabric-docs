...
- **Rename** - Changes the name of the object. Note: This change **only** affects the API schema and doesn't affect the original data source object from which it is derived.
...
Note that queries or mutations autogenerated from stored procedures can only be deleted. After deletion, they can be re-added using the **Get Data** or **Update Schema** options.
