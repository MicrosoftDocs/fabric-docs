...
Encountering a 404 error with the message 'Operation failed: The specified path does not exist' is a common issue when performing parallel data insertions into the same table using an SQL INSERT query. This can result in data loss. Our new feature, the File Output Committer Algorithm, resolves this issue, allowing customers to perform parallel data insertion seamlessly.

To access this feature, enable the `spark.sql.enable.concurrentWrites` feature flag, which is enabled by default starting from Runtime 1.2 (Spark 3.4). While this feature is also available in other Spark 3 versions, it is not enabled by default. Please note that this feature does not support parallel execution of INSERT OVERWRITE queries on the same table. For this purpose, Spark offers an alternative feature, which can be activated by configuring the `spark.sql.sources.partitionOverwriteMode` setting to [dynamic](https://spark.apache.org/docs/3.4.0/configuration.html#:~:text=spark.sql.sources.partitionOverwriteMode) 


#### Smart reads which skip files from failed jobs
In the current Spark committer system, when an insert into a table job fails but some tasks succeed, the files generated by the successful tasks coexist with those from the failed job. This can cause confusion for users as it becomes challenging to distinguish between files belonging to successful and unsuccessful jobs. Furthermore, if a job involves reading from a table while another job is concurrently inserting data into the same table, the reading job may access data that hasn't been committed yet. In the event of a write job failure, the reading job could process incorrect data.

Our new feature addresses this issue and is controlled by the `spark.sql.auto.cleanup.enabled` flag. When enabled, Spark will automatically skip reading files that have not been committed when performing `spark.read` or select queries from a table. It's important to note that files written prior to enabling this feature will still be read as usual.

Here are the visible changes:
1. All files will now include a `tid-{jobID}` identifier in their filenames.
2. Instead of the `_success` marker typically created in the output location upon successful job completion, a new `_committed_{jobID}` marker will be generated. This marker associates successful Job IDs with specific filenames.
3. We've introduced a new SQL command that users can run periodically to manage storage and clean up uncommitted files. The syntax for this command is as follows:
   ...
   In this syntax, `path/to/dir` represents the location URI where cleanup is required, and `number` is a double type value representing the retention period. The default retention period is set to 7 days.
4. We've introduced a new configuration option called `spark.sql.deleteUncommittedFilesWhileListing`, which is set to `false` by default. Enabling this option will result in the automatic deletion of uncommitted files during reads, but please note that this may slow down read operations. It is recommended to manually run the cleanup command when the cluster is idle instead of enabling this flag.
