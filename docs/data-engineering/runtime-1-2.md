...
Apache Spark 3.4.0 is the fifth release in the 3.x line. This release, driven by the open-source community, resolved over 2,600 Jira tickets. It introduces a Python client for Spark Connect, enhances Structured Streaming with async progress tracking and Python stateful processing, expands Pandas API coverage with NumPy input support, simplifies migration from traditional data warehouses through ANSI compliance and new built-in functions, and improves development productivity and debuggability with memory profiling. Additionally, Runtime 1.2 is based on Apache Spark 3.4.1, a maintenance release focused on stability fixes.
...
The `spark.sql.auto.cleanup.enabled` flag controls our new feature, addressing this issue. When enabled, Spark automatically skips reading files that have not been committed when it performs `spark.read` or selects queries from a table. Files written before enabling this feature continue to be read as usual.

Here are the visible changes:
1. All files now include a `tid-{jobID}` identifier in their filenames.
2. Instead of the `_success` marker typically created in the output location upon successful job completion, a new `_committed_{jobID}` marker is generated. This marker associates successful Job IDs with specific filenames.
...
*   In Spark 3.4, when using the Pandas on Spark API `Groupby.apply`, if the return type of the `func` parameter is not specified and `compute.shortcut_limit` is set to 0, the number of sampling rows will be automatically set to 2. This adjustment ensures that there are always at least 2 sampling rows to maintain accurate schema inference.
...
*   In Spark 3.4, the Pandas on Spark API `Index.__setitem__` will first to check `value` type is `Column` type to avoid raising unexpected `ValueError` in `is_list_like` like Can't convert column into bool: use ‘&’ for ‘and’, ‘|’ for ‘or’, ‘~’ for ‘not’ when building DataFrame boolean expressions..
