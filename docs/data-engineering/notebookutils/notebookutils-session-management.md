...
The `notebookutils.session` module provides utilities for managing the lifecycle of notebook sessions in Microsoft Fabric. You can programmatically stop an interactive session or restart the Python interpreter—useful for cleanup, resource management, and error recovery.

The following table lists the available session management methods:

| Method | Signature | Description |
|---|---|---|
| `stop` | `stop(): void` | Stops the current interactive session asynchronously and releases resources. |
| `restartPython` | `restartPython(): void` | Restarts the Python interpreter while keeping the Spark context intact. |
...
> [!NOTE]
> Code after `session.stop()` doesn't execute. All in-memory data and variables are lost after the session stops. Save important data before calling this method.

## Restart the Python interpreter

The `notebookutils.session` utility provides a way to restart the Python interpreter.
...
- In the notebook reference run case, `restartPython()` only restarts the Python interpreter of the current notebook that's being referenced.
