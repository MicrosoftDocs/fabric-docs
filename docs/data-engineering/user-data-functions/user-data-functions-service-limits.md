...
- **Service principal support**: Accessing Fabric items or data sources using a Service Principal is not currently supported through connections managed by UDF. For example, Fabric User Data Functions cannot use managed identity or workspace identity to connect to UDF supported connections.
