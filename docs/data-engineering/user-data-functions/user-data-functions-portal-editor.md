...
You can develop your Fabric User data functions directly from your browser using the portal editor. The portal allows you to create, modify, test and publish your functions without the need to install additional tooling. In this article, you will learn the following:
...
1. **Settings:** This button opens the User Data Functions item settings. This includes item description, sensitivity label, endorsement, manage connections and library management settings. The settings in this menu are applicable to all functions in this item.
1. **Functions list:** This list contains the functions inside of this User Data Functions item. If you are in **Develop mode**, this list may include new and unpublished functions. If you are in **Run/View only modes**, this list will only include published functions. Hovering over the list items reveals Run or Test functionality, depending on the mode you are in, along with additional options per function. 
...
1. **Publish button:** This button starts the publish process for your User Data Functions item. This process will publish all functions in your item. After your functions are published, other users and Fabric items can run your functions.
...
1. **Settings:** This button opens the User Data Functions item settings. This includes item description, sensitivity label, endorsement, manage connections and library management settings. The settings in this menu are applicable to all functions in this item.
1. **Refresh button:** This button refreshes the User Data Functions item with the latest published functions and metadata. You can this to ensure that you are working with the latest version of your item.
...
1. **Reset code button:** This button resets all your functions to the published version of the code. This feature will un-do any code changes that are different from the published functions. This button is only enabled in **Develop mode**.
...
#### Fabric Notebooks
This generates a code snippet to invoke your functions from a [Fabric Notebook](../notebook-utilities.md#user-data-function-udf-utilities). 
...
- **Summary:** This field provides a one-line explanation of the function. This is often displayed in a high-level view of a library or API that consumes the specification. In the OpenAPI schema, this maps directly to the "summary" field.
- **Description:** This field is used for a detailed explanation of the purpose and behavior of the function. You can use multiple lines to describe the function outputs, input, and any potential side effects or exceptions it raises. This corresponds to the "description" field in the OpenAPI specification.
