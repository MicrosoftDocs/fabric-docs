...
- Does not create physical directory structures per column value.
- Does not require you to choose clustering columns at table creation time (they can be changed later).
...
After changing clustering columns, the new layout is applied on the next `OPTIMIZE` run. Existing files retain their prior layout until they are reclustered.

## Apply clustering with OPTIMIZE

Clustering is applied during the `OPTIMIZE` command. There is no need to specify columns in the `OPTIMIZE` statement since the clustering definition is stored in table metadata:
...
1. **File selection**: The engine selects files that need clustering.
    ...
    - In **Runtime 1.3**, all files within every Z-Cube smaller than 100 GB are selected — regardless of whether they are already well-clustered.
...
The result is files with non-overlapping (or minimally overlapping) value ranges on clustering columns, enabling the engine to skip files that don't match query predicates.

> [!CAUTION]
> **Fabric Runtime 1.3 (Delta 3.2): use liquid clustering with caution.** In this runtime, liquid clustering uses a full Z-Cube rewrite strategy — every file within a Z-Cube is rewritten on every run. A Z-Cube is preserved (skipped) only when its size exceeds 100 GB. For tables smaller than 100 GB, this means every `OPTIMIZE` run rewrites all table data, even when the data is already well-clustered. This causes severe write amplification.
>
...
Starting with **Fabric Runtime 2.0** (Delta 4.1), liquid clustering uses an **incremental** clustering strategy by default. This is a significant improvement over the standard clustering behavior.
...
The standard clustering algorithm rewrites **all** files within a Z-Cube (up to 100 GB) on every `OPTIMIZE` run, regardless of whether they are already well-clustered. For a table receiving small appends, this means clustering cost grows linearly with table size, not with the amount of new data.

Incremental mode solves this by selecting only files that actually need clustering:
...
Incremental liquid clustering includes automatic overlap detection, known as **auto reclustering**, to maintain clustering quality over time. As new data arrives, it can create overlap between file value ranges, degrading data skipping effectiveness. Auto reclustering detects this and selectively reclusters only the affected files.

This process runs automatically as part of `OPTIMIZE` whenever there is new/changed data to cluster. No manual intervention or scheduled full reclusters are required — the incremental clustering strategy maintains near-optimal clustering quality as data evolves.

### Revert to full rewrite behavior

If you need to disable the incremental clustering strategy and use the full-rewrite behavior:
...
|---|---|---|---|
| `spark.microsoft.delta.optimize.clustering.strategy.incremental` | Boolean | `true` | Master switch for incremental clustering. When `true`, `OPTIMIZE` only processes unclustered, unhealthy, small, and deletion-vector files. When `false`, all files for Z-Cubes under 100GB in size are rewritten (standard behavior). |
