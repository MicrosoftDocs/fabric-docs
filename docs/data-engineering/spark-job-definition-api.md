...
1. An Entra token is required to access the Fabric Rest API. The MSAL library is recommended to get the token. For more information, refer to [Authentication flow support in MSAL](/entra/identity-platform/msal-authentication-flows).
1. A storage token is required to access the OneLake API. For more information, refer to [MSAL for Python](/entra/msal/python/).
