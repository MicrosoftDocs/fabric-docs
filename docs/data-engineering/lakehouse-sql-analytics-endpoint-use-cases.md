...
The SQL analytics endpoint is a read-optimized, T-SQL surface over Delta data in Fabric. This article explains the Fabric data warehousing workload with the [SQL analytics endpoint](lakehouse-sql-analytics-endpoint.md) of the lakehouse, and scenarios for use of the lakehouse in data warehousing.
...
- Every lakehouse has one SQL analytics endpoint, and each workspace can have more than one lakehouse. Other Fabric items - including warehouses, mirrored databases, SQL databases, and Azure Cosmos DB - also auto-provision a SQL analytics endpoint, so a workspace can have more SQL analytics endpoints than lakehouse items.
...
### Analytics with your lakehouse's gold layer
...
Data in a lakehouse in Fabric is physically stored in OneLake with the following folder structure:
...
A SQL analytics endpoint enables you to leave the data in place and still analyze data in the warehouse or lakehouse, even in other Fabric workspaces, via a seamless virtualization. Every lakehouse in Fabric stores data in OneLake.

[Shortcuts](lakehouse-shortcuts.md) enable you to reference folders in any OneLake location.

Every warehouse in Fabric stores table data in OneLake. If a table is append-only, the table data is exposed as Delta Lake data in OneLake. Shortcuts enable you to reference folders in any OneLake where the warehouse tables are exposed.
...
- [What is a lakehouse in Fabric?](lakehouse-overview.md)
- [Fabric decision guide: Choose between warehouse and lakehouse](../fundamentals/decision-guide-lakehouse-warehouse.md)
- [Bring your data to OneLake with lakehouse](../onelake/create-lakehouse-onelake.md)
- [Power BI semantic models in Fabric](../data-warehouse/semantic-models.md)
...
- [Move data from Azure SQL Database into lakehouse via copy assistant](../data-factory/tutorial-move-data-lakehouse-copy-assistant.md)
- [Connectivity to Fabric Data Warehouse](../data-warehouse/connectivity.md)
- [SQL analytics endpoint of the lakehouse](../data-warehouse/data-warehousing.md#sql-analytics-endpoint-of-the-lakehouse)
- [Query the SQL analytics endpoint or warehouse in Fabric](../data-warehouse/query-warehouse.md)
