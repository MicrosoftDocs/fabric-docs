...
Fabric records the result each time your materialized lake views refresh, whether from a schedule, a manual trigger, or an API call. The **Recent runs** page provides a single place to confirm your data is fresh, identify problems, and review details.
...
Each row represents a single refresh operation on your Lakehouse. The default columns are **Run ID**, **Run(s)**, **Status**, and **Start time**. To show more columns, select **Column Options**. Additional columns include **Refreshed materialized lake views**, **End time**, **Duration**, **Scheduled by**, and **Run type**.
...
- **Column Options** — Show or hide columns. Select **Apply** to save your selection, or select **Reset to default** to restore the original layout.
...
1. **Read the error** — Select a failed view in the lineage graph to see:
   ...
   - **Copy all to clipboard** to share the error with your team
