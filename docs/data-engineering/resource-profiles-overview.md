...
Select a resource profile through the `spark.fabric.resourceProfile` property. Setting this property applies that profile's default set of Spark and Delta Lake configurations to your Spark sessions. For example, selecting `readHeavyForPBI` enables V-Order because `spark.sql.parquet.vorder.default=true` is part of that profile's configuration set.
...
- **Auto-update variants aren't separate profiles.** `readHeavyForPBIAutoUpdate` is the auto-update form of the same `readHeavyForPBI` profile - not a new or different profile.
