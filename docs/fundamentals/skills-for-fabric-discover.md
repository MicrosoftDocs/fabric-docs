...
| Power BI | Plan, design, and manage Power BI semantic models and report definitions; query semantic models with DAX; and ask natural-language questions through FabricIQ. |
| Fabric admin and governance | Document workspaces, inventory items across tenants, and orchestrate workspace administration tasks through the `FabricAdmin` agent. |
| Migration | Port workloads to Microsoft Fabric from Azure Databricks, Azure Synapse Analytics, Azure HDInsight, and Synapse pipelines, with end-to-end orchestration through the `FabricMigrationEngineer` agent. |
| Discovery and search | Find Fabric items across workspaces. |
| Skill maintenance | Check the marketplace for updates to installed skills. |
...
To stay current with new releases, watch the [Releases page](https://github.com/microsoft/skills-for-fabric/releases) or use the `/check-updates` command if your tool supports it.
