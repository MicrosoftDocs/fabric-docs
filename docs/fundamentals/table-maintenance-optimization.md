...
description: Learn how to maintain and optimize Delta tables across Spark, SQL analytics endpoint, Power BI Direct Lake, Fabric Data Warehouse, and mirroring workloads in Microsoft Fabric.
...
#customer intent: As a Fabric data engineer, I want to understand how to maintain and optimize Delta tables across Fabric workloads so that I can share one table efficiently across multiple engines.
---

# Cross-workload table maintenance and optimization in Microsoft Fabric

Delta tables in Microsoft Fabric can serve Spark, SQL analytics endpoint, Power BI Direct Lake, Fabric Data Warehouse, and other Fabric experiences from data stored in OneLake. Optimal cross-workload performance depends on two things: the workload that creates and maintains the table, and the engines that consume it.

Spark is the most common lakehouse writer and provides the broadest layout and maintenance controls, while Fabric Data Warehouse and database mirroring manage their physical layouts automatically and mirrored catalogs retain the layout managed in the source system. Consumer requirements are generally compatible, so you can share a single table whenever those requirements align. This article explains how to identify which workload owns a table's layout and how to apply the right maintenance strategy for each producer and consumer across Fabric workloads.

## Understand layout ownership

Start by identifying which workload owns the physical table layout. The following table shows the key controls relevant to cross-workload table layout and maintenance, not an exhaustive list of each engine's capabilities.
...
| Lakehouse: Spark writer | Power BI Direct Lake | Use the same layout recommended for Spark and enable [V-Order](../data-engineering/delta-optimization-and-v-order.md), or use the [`readHeavyForPBI` resource profile](../data-engineering/configure-resource-profile-configurations.md#available-resource-profiles). |
...
> [!IMPORTANT]
> [Deletion vectors](../data-engineering/delta-lake-deletion-vectors.md) require Delta reader version 3 and writer version 7. Before you enable them on a shared table, verify that every consuming engine and external reader supports the required Delta protocol. A reader that can't interpret the protocol can't query the table.
...
[`OPTIMIZE`](../data-engineering/table-compaction.md#optimize-command) compacts files and automatically purges a file's deletion vectors when they reference more than 5% of its records. Use [`REORG TABLE ... APPLY (PURGE)`](../data-engineering/delta-lake-deletion-vectors.md#use-reorg-purge-to-remove-accumulated-deletion-vectors) only when you must physically purge records below that threshold or meet a specific compliance requirement.
...
Use the strategic controls that the warehouse exposes to tune the data layout:
...
For complete Fabric Data Warehouse guidance, see [Performance guidelines in Fabric Data Warehouse](../data-warehouse/guidelines-warehouse-performance.md).
...
- **[Open mirroring](../mirroring/open-mirroring.md)**: Fabric replicates data that an external application writes to a landing zone into system-managed Delta tables in OneLake. As with database mirroring, Fabric manages the file layout and maintenance, and these tables follow the same mirrored-database maintenance guidance.
...
- **File and row-group layout**: Avoid small [row groups](direct-lake-understand-storage.md#row-group-size) and uneven row group distribution, which create more VertiPaq column segments and increase transcoding overhead.
