...
This article helps you understand how Copilot in Fabric works, including its architecture and cost. The information in this article is intended to guide you and your organization to use and manage Copilot effectively. This article is primarily targeted at:
...
Enabling and effectively using Copilot in your organization requires deliberate thought and planning. Generative AI is an evolving technology with specific nuances and considerations to keep in mind, so it's important to apply generative AI tools like Copilot to the appropriate problems and scenarios. Furthermore, Copilot in Power BI consumes your available Fabric capacity, so you should manage its usage to avoid overconsumption that can lead to throttling and disruption of your other Fabric operations.
...
You can enable and use Copilot across the various Fabric workloads. Within those workloads, you can use different Copilot experiences. While each of these Copilot experiences uses a similar common architecture, they work in distinct ways.
...
Copilot for Data Engineering and Data Science is an AI-enhanced toolset tailored to support data professionals in their workflow. It provides intelligent code completion, automates routine tasks, and supplies industry-standard code templates to facilitate building robust pipelines and crafting complex analytical models. When you use advanced machine learning algorithms, Copilot offers contextual code suggestions that adapt to the specific task at hand, helping you code more effectively and with greater ease. From data preparation to insight generation, Microsoft Fabric Copilot acts as an interactive aide, lightening the load on engineers and scientists and expediting the journey from raw data to meaningful conclusions.

Copilot in notebooks is immediately context-aware without requiring you to start a session. It understands the current workspace, attached Lakehouse schemas, tables, and files, the notebook's structure, and the runtime state. This grounding allows Copilot to provide relevant suggestions and responses from the moment you open the chat pane.

Beyond individual cells, Copilot supports notebook-wide, multi-step assistance. It can generate, refactor, summarize, and validate code across the entire notebook workflow, helping you streamline end-to-end tasks and refactor code into reusable functions.

Copilot also surfaces performance insights based on data size, join patterns, and runtime behavior. For example, it can recommend efficient join strategies, identify potential data shuffles to avoid, and flag data quality issues.

When a cell or Spark job fails, the **Fix with Copilot** feature (preview) provides an error summary, root-cause analysis, and recommended fixes. Copilot can auto-apply code changes with an approval diff so you can review before committing. You can also invoke targeted diagnostics by using the `/fix` command in Copilot chat for a specific cell or the entire notebook.
...
The Fabric portal Query Editor includes a built-in Copilot experience for SQL database that uses the same underlying tooling as the SQL Server Management Studio (SSMS) and Visual Studio Code integrations to provide consistent chat and inline assistance.
...
Copilot is available when connecting to a Fabric SQL database from external tools such as SQL Server Management Studio (SSMS) 22 and the MSSQL extension for Visual Studio Code, offering inline T‑SQL assistance and chat-based help grounded to the connected database. These integrations allow you to use Copilot's capabilities while working with your SQL database in the tools you prefer, providing flexibility and convenience in how you interact with your data.

- **SQL Server Management Studio (SSMS)**: When you connect to a SQL database in Fabric from SSMS, you can use Copilot for SQL database features such as code generation, code completion, quick actions, and document-based Q&A. Additionally, you can get inline T-SQL completions and chat-based assistance grounded to the connected database, and analyze execution plans with Copilot's help.
...
Power BI introduces generative AI that you can use to create reports automatically by selecting the article for a report or by prompting Copilot for Power BI on a particular article. Use Copilot for Power BI to generate a summary for the report page that you created, and generate synonyms for better Q&A capabilities. For more information on the features and how to use Copilot for Power BI, see [Overview of Copilot for Power BI](/power-bi/create-reports/copilot-introduction).
...
Copilot is available both in the KQL queryset editor and inside dashboard tile editing, allowing you to write natural language prompts to generate, replace, or refine KQL queries. This approach gives you flexibility to author queries in the context that best fits your workflow - whether building queries in a queryset or tuning visuals directly within dashboard tiles.
...
| | Real-time dashboards | - [Generate a real-time dashboard](../real-time-intelligence/copilot-generate-dashboard.md).<br />- [Edit tile with Copilot (preview)](../real-time-intelligence/dashboard-real-time-create.md#addedit-tile).<br />- [Author queries with Copilot (preview)](../real-time-intelligence/copilot-writing-queries.md). |
> [!NOTE]  
...
The Azure OpenAI Service that powers Fabric Copilot is currently deployed in US datacenters (East US, East US2, South Central US, and West US) and in the EU data boundary (France Central and Sweden Central). This deployment differs from the standard Azure OpenAI Service available in the Azure portal, which is accessible in many more regions. For details on standard Azure OpenAI region availability, see [Azure OpenAI Service region availability](/azure/ai-services/openai/concepts/models?tabs=global-standard%2Cstandard-chat-completions#model-summary-table-and-region-availability). If your capacity's geographic region is outside the US or the EU data boundary, Copilot is disabled by default unless your Fabric admin enables the **Data sent to Azure OpenAI can be processed outside your capacity's geographic region, compliance boundary, or national cloud instance** tenant setting. To learn how to get to the tenant settings, see [About tenant settings](../admin/service-admin-portal-copilot.md).
