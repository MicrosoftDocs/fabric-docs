...
OneLake can detect the data type referenced by a shortcut and apply either [file transformations](../onelake/shortcuts/transformations.md) or [AI transformations](../onelake/transformations-ai.md) without requiring a pipeline or custom code. OneLake maintains the resulting Delta table in sync with the source automatically. For example, you can convert `.csv` files to Delta tables or apply AI-based sentiment analysis to `.txt` files in a folder.
