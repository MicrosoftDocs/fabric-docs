...
Real-Time Intelligence with Eventhouse provides a unified analytics surface that spans KQL, T-SQL through the SQL analytics endpoint, and notebooks, enabling end-to-end real-time-to-historical analysis on the same data without copies or data movement.
...
Eventhouse also exposes a managed [SQL analytics endpoint](../real-time-intelligence/eventhouse-analyze-data-with.md) aligned with the Eventhouse data model and Fabric governance. The endpoint provides familiar T-SQL access (for example, T-SQL queries and connectivity from BI tools) on Eventhouse data without data movement.

In addition, Eventhouse integrates with [Fabric notebooks](../real-time-intelligence/notebooks.md), which support KQL, T-SQL, Python, and Spark. You can use notebooks to explore, transform, and build machine learning workflows on the same Eventhouse data, keeping work within Fabric and avoiding copies.
...
The [anomaly detector](../real-time-intelligence/anomaly-detection.md) automatically identifies unusual patterns or outliers in streaming or time-series data. Detection runs natively against live Eventhouse tables and incoming streaming data without copying or exporting data, supporting both real-time detection and evaluations over historical windows. It works by analyzing data ingested into Eventhouse (KQL databases) and [applying statistical models or machine learning techniques](../real-time-intelligence/anomaly-detection-models.md) to detect deviations from expected behavior. These anomalies could represent system failures, fraud, performance degradation, or other critical events that require attention. Unlike static threshold-based alerts, the anomaly detector adapts to the data's historical trends and seasonality, making it more effective at identifying subtle or context-sensitive issues.
...
Beyond business entities and relationships, ontologies can also define actionable rules, such as "If inventory < threshold, trigger replenishment." [Operations Agents](../real-time-intelligence/operations-agent.md) use these rules to trigger workflows in Activator. Operations Agents can reason over both live streaming and historical data in Eventhouse mapped through the ontology, and they can invoke Activator with context derived from Eventhouse-backed entities. When an Operations Agent invokes [Activator to run a Power Automate flow](../real-time-intelligence/data-activator/activator-trigger-power-automate-flows.md), it passes parameters derived from ontology properties, such as CustomerID and OrderStatus. This approach ensures that automation flows operate with full business context, not just raw IDs.
