...
|Users can create Plan items|Users can create a Plan in Fabric and leverage no-code integrated planning using Power BI Semantic models, Fabric SQL and OneLake.  [Learn More](https://aka.ms/planningdocs)|
...
|Enable anonymous data access for Fabric Apps (Preview)|This setting allows users to enable anonymous data access for Fabric Apps that are intended for public consumption. Admins can limit this capability to specific users or security groups.<br><br>Anonymous data access lets users on the internet view and query data exposed by your Fabric Apps without signing in. Only enable this for apps that are explicitly intended for public consumption, and limit who can enable it by selecting specific security groups.|
|Users can create deployment plan items|Turn on this setting to allow users to define the order, dependencies, and actions for deploying workspace items across environments.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2301210)|
...
|[Guest users can work with shared semantic models in their own tenants](/power-bi/collaborate-share/service-dataset-external-org-share-admin#allow-guest-users-to-work-with-shared-datasets-in-their-own-tenants)|Authorized guest users can discover semantic models shared with them in the OneLake catalog (in Power BI Desktop), and then work with these semantic models in their own Power BI tenants.|
