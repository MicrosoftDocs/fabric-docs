...
| Setting name | Description |
|------|-------|
|Block republish and disable package refresh|Disable package refresh, and only allow the semantic model owner to publish updates. When using [XML/A Endpoint](/power-bi/enterprise/service-premium-connect-tools), a user can discover only those semantic models for which they are the owner.|
