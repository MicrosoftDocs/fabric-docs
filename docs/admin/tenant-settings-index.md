...
|Enable Event Schema Sets in Real-Time Hub (preview)|This setting allows users to define the structure and format of events through schemas, enabling data quality and validation with group-level access control. [Learn More](https://aka.ms/AD_docs)|
...
|Enable new mirrored catalog items (preview)|Turn on this setting to allow users to create mirrored catalog items in Fabric, to mirror tables from external catalogs into OneLake through shortcuts. This only affects mirrored catalog item types that are in a Preview state.  [Learn More](https://aka.ms/OneLakeMirroredCatalogDocs)|
|Enable Fabric App Items (preview)|Users can create Fabric apps which is a Fabric-native suite of services for building intelligent applications for front end and backend of your application. It allows developers and AI agents to create and operate complete app backends directly on Microsoft Fabric. Fabric Apps unifies operational data, analytics, and AI while meeting enterprise security and governance requirements. [Learn More](https://go.microsoft.com/fwlink/?linkid=2353418)|
|Enable anonymous data access for Fabric Apps (Preview)|This setting allows users to enable anonymous data access for Fabric Apps that are intended for public consumption. Admins can limit this capability to specific users or security groups.|
|Users can create Azure Databricks Storage items (preview)|Users can create Azure Databricks storage item that uses OneLake for storage.|
|Enable sending Operations Agents observability data to Agent 365 (preview)|Allow Microsoft Fabric Operation Agent data to be processed and stored by Agent 365, such as information about the Fabric item for this agent, the user ID of the creator of the agent, the Entra Agent ID that the agent is operating with, and the prompts and tools that it uses. This will help Agent 365 administrators monitor what Fabric agents are being created, where they are being used and more.  When this setting is turned on, data sent to Agent 365 can be processed and/or stored outside your capacity's geographic region, compliance boundary, or national cloud instance.   [Learn More](https://go.microsoft.com/fwlink/?LinkId=2365600)|
|Insights for operations agents (preview)|Users can view operational insights for events generated by operations agents.<br><br>These insights help users understand what happened and why by showing summarized findings and key observations based on agent activity and underlying data.  [Learn More](https://aka.ms/OA_RCA_learnmore)<br><br>Turning on this setting can help your organization improve issue triage by identifying possible causes behind events, understand the context behind agent recommendations, and increase transparency into how agents analyze and act on data.|
|Fabric data agents can send operational metadata for observability in Microsoft Foundry|Agent creators can use operational metadata to monitor agent health, detect issues, and understand how their published agents are used across applications and services. These insights help agent creators improve agent reliability and performance.<br><br>When this setting is turned on, operational metadata from Fabric data agents is processed and stored in a configured Application Insights resource. This data includes agent display names, data source names, reasoning step IDs, and conversation IDs. The Application Insights resource that processes and stores this data may be located outside your tenant's geographic region, compliance boundary, or national cloud instance.<br><br>Accessing the operational metadata requires a role with read permissions (such as Monitoring Reader or Log Analytics Reader) on the configured Application Insights resource. Users with this access can view the data in Application Insights. Users can also view the data through a Foundry project that connects to the configured Application Insights resource. Access through Foundry requires role-based access to the Foundry project (Foundry User or higher) in addition to read permissions on the Application Insights resource. Access to a Foundry project alone doesn't include access to the telemetry data. [Learn More](https://aka.ms/Fabric/Data-Agent-Foundry-Observability)|
...
|Receive In-product notifications for service outages or incidents|Mail-enabled security groups will receive In-product notifications if this tenant is impacted by a service outage or incident. [Learn More](https://go.microsoft.com/fwlink/?linkid=2347823)|
...
|[Users can send email subscriptions to external users](./service-admin-portal-export-sharing.md#users-can-send-email-subscriptions-to-external-users)|Users can send email subscriptions to external users. External users are users you've not added to your Microsoft Entra ID. Turn this setting off to prevent users from subscribing external users to email subscriptions.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2256884)|
...
|Users with read or write permission can download data from notebooks|With this setting on, users with read or write permission can download or export data from tables in notebook outputs.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2299416)|
...
|Users can find objects in search|Include objects such as tables and columns in the OneLake catalog and global search. Users can find objects only if they have access to items that contain them. Disable to limit discovery to top-level items, like lakehouses and reports.  [Learn More](https://go.microsoft.com/fwlink/?LinkId=2370598)|
...
|[Enable Denodo SSO](/power-query/connectors/denodo)|Enable SSO capability for Denodo. By enabling, user access token information, including name and email, will be sent to Denodo for authentication.|
|[Enable Starburst SSO](/power-query/connectors/starburst)|Enable SSO capability for Starburst. By enabling, user access token information, including name and email, will be sent to Starburst for authentication.|
|Microsoft Entra single sign-on for data gateway|Users can use Microsoft Entra single sign-on (SSO) to authenticate to on-premises / virtual network data gateways and access data sources.<br><br>With this setting on, user access token information, including names and emails, is sent to data sources to authenticate to the on-premises / virtual network data gateway service.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2256374)|
|Microsoft Entra single sign-on for virtual network (VNet) data gateway|Users can use Microsoft Entra single sign-on (SSO) to authenticate to virtual network (VNet) data gateways and access data sources.<br><br>With this setting on, user access token information, including names and emails, is sent to data sources to authenticate to the virtual network data gateway service.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2256374)|
...
|Users can connect to data sources by using Apache Arrow database connectivity (ADBC)|ADBC offers enhanced performance, security, and reliability compared to legacy connectivity options. When enabled, data connections to supported data sources use ADBC by default.  [Learn More](https://aka.ms/ADBCOverview)|
...
| Setting name | Description |
|------|-------|
|[Usage metrics for content creators](/power-bi/collaborate-share/service-modern-usage-metrics)|Users in the organization can see usage metrics for dashboards, reports and semantic models that they have appropriate permissions to.  [Learn More](https://go.microsoft.com/fwlink/?linkid=2352692)|
...
|[Define maximum number of Fabric identities in a tenant](service-admin-portal-developer.md#define-maximum-number-of-fabric-identities-in-a-tenant)|Allow admins to specify the maximum number of Fabric identities that can be created in a tenant. If this setting is disabled, up to 10000 Fabric identities can be created in a tenant.  [Learn more](https://go.microsoft.com/fwlink/?linkid=2339322).|
...
|Configure workspace-level IP firewall rules and trusted resource instances|With this setting enabled, workspace admins can configure IP firewall rules and trusted resource instances in Workspace settings. Even when the tenant blocks public access, this setting still allows workspace admins to grant access from specific IP firewall rules and designated trusted resource instances. [Learn More](https://go.microsoft.com/fwlink/?linkid=2223103)|
...
| Setting name | Description |
|------|-------|
|Allow Real-Time Dashboards to include embedded app visuals|Enabling this setting allows dashboards to accept embedded app visuals, which are external apps embedded as iframes within Real-Time Dashboards|
...
|Users can use Fabric Copilot (preview)|When this setting is on, users can create new Fabric Copilot sessions and send prompts in existing sessions. When this setting is off, users can't create new sessions or send new prompts and existing session history remains read-only. By turning on this setting, you agree to the Preview Terms for any AI features in preview.|
...
| Setting name | Description |
|------|-------|
|Allow data processing outside geographic region for Azure AI services|This setting applies only to customers using prebuilt AI services in Fabric whose capacity is in a region not covered by Azure AI services.  [Learn More](https://aka.ms/Fabric/AI-Serivce/Regions)<br><br>When enabled, data sent to Azure AI services may be processed outside your capacity's geographic region, compliance boundary, or national cloud instance. <br><br>Check the  [documentation](https://aka.ms/Fabric/AI-Service/Data) for details about what data types are affected. You can manage this setting at both tenant and capacity levels.  [Learn More](https://aka.ms/Fabric/AI-Service/Tenant-Setting)|
