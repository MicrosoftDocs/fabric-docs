...
> [!NOTE]
> If the workspace has a [workspace identity](../security/workspace-identity.md), the identity isn't deleted while the workspace is in its retention period. Its service principal and app registration remain visible in the Azure portal until the workspace is permanently deleted. For more information, see [Deleting the identity](../security/workspace-identity.md#deleting-the-identity).
